...
/**
 * Description of DB Change Log Controller
 *
 * @category admin
 *
 ...
 *
 * @module DB Change Log
 *
 * @class Db_change_log.php
 *
 * @path application\admin\notifications\controllers\Db_change_log.php
 *
 * @version 4.3
 *
 * @author CIT Dev Team
...
class Db_change_log extends Cit_Controller
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->load->model('db_change_log_model');
        $this->_request_params();
        $this->folder_name = "notifications";
        $this->module_name = "db_change_log";
        ...
        $this->dropdown_arr = array(
            "mdc_operation" => array(
                ...
                "values" => array(
                    array(
                        'id' => 'Added',
                        'val' => $this->lang->line('DB_CHANGE_LOG_ADDED')
                    ),
                    array(
                        'id' => 'Modified',
                        'val' => $this->lang->line('DB_CHANGE_LOG_MODIFIED')
                    ),
                    array(
                        'id' => 'Deleted',
                        'val' => $this->lang->line('DB_CHANGE_LOG_DELETED')
                    )
                )
            ),
            "mdc_source" => array(
                ...
                "values" => array(
                    array(
                        'id' => 'Admin',
                        'val' => $this->lang->line('DB_CHANGE_LOG_ADMIN')
                    ),
                    array(
                        'id' => 'Front',
                        'val' => $this->lang->line('DB_CHANGE_LOG_FRONT')
                    ),
                    array(
                        'id' => 'API',
                        'val' => $this->lang->line('DB_CHANGE_LOG_API')
                    ),
                    array(
                        'id' => 'Notification',
                        'val' => $this->lang->line('DB_CHANGE_LOG_NOTIFICATION')
                    ),
                    array(
                        'id' => 'Other',
                        'val' => $this->lang->line('DB_CHANGE_LOG_OTHER')
                    )
                )
            )
        );
        ...
        try
        {
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                $access_list = array(
                    ...
                    "db_change_log_view",
                    "db_change_log_add",
                    "db_change_log_update",
                    "db_change_log_delete",
                    "db_change_log_export",
                    "db_change_log_print",
                );
            ...
            else
            {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("db_change_log", $access_list, TRUE, TRUE);
            }
            if (!$list_access)
            {
                throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_VIEW_THIS_PAGE_C46_C46_C33'));
            }
            $enc_loc_module = $this->general->getMD5EncryptString("ListPrefer", "db_change_log");
            ...
            $status_label = array(
                'js_lang_label.DB_CHANGE_LOG_ADMIN',
                'js_lang_label.DB_CHANGE_LOG_FRONT',
                'js_lang_label.DB_CHANGE_LOG_API',
                'js_lang_label.DB_CHANGE_LOG_NOTIFICATION',
                'js_lang_label.DB_CHANGE_LOG_OTHER',
            );

            $list_config = $this->db_change_log_model->getListConfiguration();
            $this->processConfiguration($list_config, $add_access, $edit_access, TRUE);
            $this->general->trackModuleNavigation("Module", "List", "Viewed", $this->mod_enc_url["index"], "db_change_log");
            ...
            $render_arr = array(
                ...
                'default_filters' => $this->db_change_log_model->default_filters,
            );
            ...
            {
                $this->loadView("db_change_log_index");
            }
        }
        catch(Exception $e)
        {
    ...
    public function listing()
    {
        ...
        $list_config = $this->db_change_log_model->getListConfiguration();
        $form_config = $this->db_change_log_model->getFormConfiguration();
        $extra_cond = $this->db_change_log_model->extra_cond;
        $groupby_cond = $this->db_change_log_model->groupby_cond;
        $having_cond = $this->db_change_log_model->having_cond;
        $orderby_cond = $this->db_change_log_model->orderby_cond;
        ...
        $data_recs = $this->db_change_log_model->getListingData($data_config);
        $data_recs['no_records_msg'] = $this->general->processMessageLabel('ACTION_NO_DB_CHANGE_LOG_DATA_FOUND_C46_C46_C33');
        if (method_exists($this->general, 'dbLogDataBeforeLoad'))
        {
            $data_recs = $this->general->dbLogDataBeforeLoad($data_recs, $this->db_change_log_model->grid_fields, $list_config);
        }
    ...
    public function export()
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
        {
            $this->filter->checkAccessCapability("db_change_log_export");
        }
        else
        {
            $this->filter->getModuleWiseAccess("db_change_log", "Export", TRUE);
        }
        $params_arr = $this->params_arr;
        $page = $params_arr['page'];
        ...
        $list_config = $this->db_change_log_model->getListConfiguration();
        $form_config = $this->db_change_log_model->getFormConfiguration();
        $table_name = $this->db_change_log_model->table_name;
        $table_alias = $this->db_change_log_modeltable_alias;
        $primary_key = $this->db_change_log_model->primary_key;
        $extra_cond = $this->db_change_log_model->extra_cond;
        $groupby_cond = $this->db_change_log_model->groupby_cond;
        $having_cond = $this->db_change_log_model->having_cond;
        $orderby_cond = $this->db_change_log_model->orderby_cond;
        ...
        $db_recs = $this->db_change_log_model->getExportData($export_config);
        ...
        $misc_info['heading'] = $this->lang->line('DB_CHANGE_LOG_DB_CHANGE_LOG');
        $misc_info['filename'] = "db_change_log_records_".count($db_recs);
        ...
        if ($export_type == 'pdf')
        {
            ...
            {
                $pdf->setModule("db_change_log_model");
            }
        ...
        try
        {
            ...
            if ($mode == "Update")
            {
                ...
                else
                {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("db_change_log", $access_list, TRUE, TRUE);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("db_change_log", $access_list, TRUE, TRUE);
                }
                if (!$add_access)
                {
            ...
            if ($mode == 'Update')
            {
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowEdit", "db_change_log"), $this->session->userdata('iAdminId'));
                $data_arr = $this->db_change_log_model->getData(intval($id));
                ...
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "db_change_log", $recName);
            }
            else
            {
                $recName = '';
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowAdd", "db_change_log"), $this->session->userdata('iAdminId'));
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "db_change_log");
            }

            $opt_arr = $img_html = $auto_arr = $config_arr = array();

            $form_config = $this->db_change_log_model->getFormConfiguration($config_arr);
                ...
                foreach ($form_config as $key => $val)
                {
                    ...
                    if ($val['function'] != "")
                    {
                        ...
                        elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                        {
                            ...
                            if (method_exists($this->db_change_log_model, $phpfunc))
                            {
                                $tmpdata = $this->db_change_log_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                        }
                    ...
                    if ($val['field_status'] != "")
                    {
                        ...
                        elseif ($status_type == "function")
                        {
                            ...
                            elseif (substr($fd_callback, 0, 7) == 'model::' && substr($fd_callback, 7) !== FALSE)
                            {
                                ...
                                if (method_exists($this->db_change_log_model, $fd_callback))
                                {
                                    $fd_status = $this->db_change_log_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
                            }
            ...
            else
            {
                if ($mode == "Update")
                {
                    ...
                    {
                        $this->loadView("db_change_log_add");
                    }
                    else
                    {
                        $this->loadView("db_change_log_add_view");
                    }
                }
                else
                {
    ...
    public function addAction()
    {
        ...
        try
        {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                if ($mode == "Update")
                {
                    $add_edit_access = $this->filter->checkAccessCapability("db_change_log_update", TRUE);
                }
                else
                {
                    $add_edit_access = $this->filter->checkAccessCapability("db_change_log_add", TRUE);
                }
            }
            else
            {
                $add_edit_access = $this->filter->getModuleWiseAccess("db_change_log", $mode, TRUE, TRUE);
            }
            if (!$add_edit_access)
            {
                if ($mode == "Update")
            ...
            $mdc_table_name = $params_arr["mdc_table_name"];
            ...
            $mdc_date_added = $params_arr["mdc_date_added"];

            $data = $save_data_arr = $file_data = array();
            $data["vTableName"] = $mdc_table_name;
            $data["eOperation"] = $mdc_operation;
            $data["iPrimaryKey"] = $mdc_primary_key;
            $data["tFieldData"] = $mdc_field_data;
            $data["eSource"] = $mdc_source;
            $data["iLoggedById"] = $mdc_logged_by_id;
            $data["vLoggedName"] = $mdc_logged_name;
            $data["dDateAdded"] = $this->filter->formatActionData($mdc_date_added, $form_config["mdc_date_added"]);

            $save_data_arr["mdc_table_name"] = $data["vTableName"];
            $save_data_arr["mdc_operation"] = $data["eOperation"];
            $save_data_arr["mdc_primary_key"] = $data["iPrimaryKey"];
            $save_data_arr["mdc_field_data"] = $data["tFieldData"];
            $save_data_arr["mdc_source"] = $data["eSource"];
            $save_data_arr["mdc_logged_by_id"] = $data["iLoggedById"];
            $save_data_arr["mdc_logged_name"] = $data["vLoggedName"];
            $save_data_arr["mdc_date_added"] = $data["dDateAdded"];
            if ($mode == 'Add')
            {
                $id = $this->db_change_log_model->insert($data);
                if (intval($id) > 0)
                {
                    $save_data_arr["iDBChangelogId"] = $data["iDBChangelogId"] = $id;
                ...
                $track_cond = $this->db->protect("mdc.iDBChangelogId")." = ".$this->db->escape($id);
                $switch_combo = $this->db_change_log_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Added", $this->mod_enc_url["add"], "db_change_log", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            elseif ($mode == 'Update')
            {
                $res = $this->db_change_log_model->update($data, intval($id));
                if (intval($res) > 0)
                {
                    $save_data_arr["iDBChangelogId"] = $data["iDBChangelogId"] = $id;
                ...
                $switch_combo = $this->db_change_log_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Modified", $this->mod_enc_url["add"], "db_change_log", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            $ret_arr['id'] = $id;
            $ret_arr['mode'] = $mode;
            $ret_arr['message'] = $msg;
    ...
    public function inlineEditAction()
    {
        ...
        if ($all_row_selected == "true" && in_array($operartor, array("del", "status")))
        {
            ...
            $config_arr['list_config'] = $this->db_change_log_model->getListConfiguration();
            $config_arr['form_config'] = $this->db_change_log_model->getFormConfiguration();
            $config_arr['table_name'] = $this->db_change_log_model->table_name;
            $config_arr['table_alias'] = $this->db_change_log_model->table_alias;
        ...
        if ($search_alias == "Yes")
        {
            $primary_field = $this->db_change_log_model->table_alias.".".$this->db_change_log_model->primary_key;
        }
        else
        {
            $primary_field = $this->db_change_log_model->primary_key;
        }
        ...
        try
        {
            switch ($operartor)
            {
                case 'del':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $del_access = $this->filter->checkAccessCapability("db_change_log_delete", TRUE);
                    }
                    else
                    {
                        $del_access = $this->filter->getModuleWiseAccess("db_change_log", "Delete", TRUE, TRUE);
                    }
                    if (!$del_access)
                    {
                    ...
                    $success = $this->db_change_log_model->delete($extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_FAILURE_IN_DELETION_THIS_RECORD_C46_C46_C33'));
                    }
                ...
                case 'edit':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $edit_access = $this->filter->checkAccessCapability("db_change_log_update", TRUE);
                    }
                    else
                    {
                        $edit_access = $this->filter->getModuleWiseAccess("db_change_log", "Update", TRUE, TRUE);
                    }
                    if (!$edit_access)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_MODIFY_THESE_DETAILS_C46_C46_C33'));
                    ...
                    $list_config = $this->db_change_log_model->getListConfiguration($post_name);
                    $form_config = $this->db_change_log_model->getFormConfiguration($list_config['source_field']);
                    if (!is_array($form_config) || count($form_config) == 0)
                    {
                    ...
                    $success = $this->db_change_log_model->update($data_arr, intval($primary_ids));
                ...
                case 'status':
                    ...
                    {
                        $field_name = $this->db_change_log_model->table_alias.".eSource";
                    }
                    else
                    {
                    ...
                    $success = $this->db_change_log_model->update($data_arr, $extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
    ...
    public function getSourceOptions($name = '', $mode = 'Add', $id = '', $data = array(), $extra = '', $rtype = 'records')
    {
        ...
        switch ($type)
        {
            ...
            case 'table':
                ...
                {
                    $combo_config['main_table'] = $this->db_change_log_model->table_name;
            ...
            case 'phpfn':
                ...
                elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                {
                    ...
                    if (method_exists($this->db_change_log_model, $phpfunc))
                    {
                        $data_arr = $this->db_change_log_model->$phpfunc($data[$name], $mode, $id, $data, $parent_src, $this->term);
                    }
                }
                elseif (method_exists($this->general, $phpfunc))
                {
    ...
    public function getSelfSwitchTo()
    {
        ...
        $switchto_fields = $this->db_change_log_model->switchto_fields;
        $extra_cond = $this->db_change_log_model->extra_cond;
        ...
        $switch_arr = $this->db_change_log_model->getSwitchTo($extra_cond);
    ...
    public function getListOptions()
    {
        ...
        $config_arr = $this->db_change_log_model->getListConfiguration($alias_name);
