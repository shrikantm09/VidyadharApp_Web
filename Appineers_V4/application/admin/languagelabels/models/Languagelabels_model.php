...
class Languagelabels_model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        $this->table_alias = "mll";
        $this->primary_key = "iLanguageLabelId";
        $this->primary_alias = "mll_language_label_id";
    ...
    public function getData($extra_cond = "", $fields = "", $order_by = "", $group_by = "", $limit = "", $join = "No", $having_cond = '', $list = FALSE)
    {
        if (is_array($fields)) {
            ...
            $this->db->select("mll.vLabel AS mll_label");
            $this->db->select("mlll.vTitle AS mlll_title");
            $this->db->select("mll.vModule AS mll_module");
            $this->db->select("mll.eStatus AS mll_status");
        } else {
            $this->db->select("mll.iLanguageLabelId AS iLanguageLabelId");
            $this->db->select("mll.iLanguageLabelId AS mll_language_label_id");
            $this->db->select("mll.vLabel AS mll_label");
            $this->db->select("mll.vPage AS mll_page");
            $this->db->select("mll.vModule AS mll_module");
            $this->db->select("mll.eStatus AS mll_status");
        }
    ...
    public function addJoinTables($type = 'AR', $allow_tables = FALSE)
    {
        ...
        $lang_arr = array(
            array(
                ...
                "join_type" => "left",
                "extra_condition" => $this->db->protect("mlll.vLangCode") . " = " . $this->db->escape($lang_code)
            )
        );
    ...
    public function getListConfiguration($name = "")
    {
        $list_config = array(
            "mll_label" => array(
                ...
                "table_alias" => "mll",
                "field_name" => "vLabel",
                "source_field" => "mll_label",
                "display_query" => "mll.vLabel",
                "entry_type" => "Table",
                "data_type" => "varchar",
                "show_in" => "Both",
                "type" => "textbox",
            ...
            "mll_module" => array(
                ...
                "table_alias" => "mll",
                "field_name" => "vModule",
                "source_field" => "mll_module",
                "display_query" => "mll.vModule",
                "entry_type" => "Table",
                "data_type" => "varchar",
                "show_in" => "Both",
                "type" => "dropdown",
                ...
                "editable" => "No",
                "viewedit" => "No",
                "default" => $this->filter->getDefaultValue("mll_module", "Text", "Generic")
            ),
            "mll_status" => array(
                "name" => "mll_status",
                "table_name" => "mod_language_label",
                "table_alias" => "mll",
                "field_name" => "eStatus",
                "source_field" => "mll_status",
                "display_query" => "mll.eStatus",
                "entry_type" => "Table",
                "data_type" => "enum",
                "show_in" => "Both",
                "type" => "dropdown",
                ...
                "editable" => "No",
                "viewedit" => "No",
                "default" => $this->filter->getDefaultValue("mll_status", "Text", "Active")
            )
        );
    ...
    public function getFormConfiguration($name = "")
    {
        $form_config = array(
            ...
            "mll_page" => array(
                ...
                "default" => $this->filter->getDefaultValue("mll_page", "Text", "Messages"),
                "dfapply" => "addOnly",
            ),
            "mll_module" => array(
                "name" => "mll_module",
    ...
    public function getLangData($extra_cond = "", $fields = "", $order_by = "", $group_by = "", $limit = "", $lang_assoc = TRUE)
    {
        if (is_array($fields)) {
            ...
            $this->db->select("mlll_lang.*");
        }
        $this->db->from("mod_language_label_lang AS mlll_lang");

        if (is_array($extra_cond) && count($extra_cond) > 0) {
            $this->listing->addWhereFields($extra_cond);
        } elseif ($extra_cond != "") {
            if (is_numeric($extra_cond)) {
                $this->db->where("mlll_lang." . $this->primary_key, $extra_cond);
            } else {
                $this->db->where($extra_cond, FALSE, FALSE);
            }
        }
        $this->general->getPhysicalRecordWhere("mod_language_label_lang", "mlll_lang", "AR");
        if ($group_by != "") {
            $this->db->group_by($group_by);
        }
        if ($order_by != "") {
    ...
    public function getLanguageLabelLangData($extra_cond = "", $fields = "", $order_by = "", $group_by = "", $limit = "", $join = "No", $assoc_field = '')
    {
        ...
        if (is_array($extra_cond) && count($extra_cond) > 0) {
            ...
            $this->db->where("mlll_lang.iLanguageLabelId", intval($extra_cond));
        } elseif ($extra_cond != '') {
            $this->db->where($extra_cond, FALSE, FALSE);
        }
        $this->general->getPhysicalRecordWhere("mod_language_label_lang", "mlll_lang", "AR");
    ...
    public function getLanguageData($extra_cond = "", $fields = "", $order_by = "", $group_by = "", $limit = "", $join = "No", $assoc_field = '')
    {
        ...
        $this->db->from("mod_language AS ml");
        ...
        $this->general->getPhysicalRecordWhere("mod_language", "ml", "AR");
        if ($group_by != "") {
            $this->db->group_by($group_by);
        }
        if ($order_by != "") {
