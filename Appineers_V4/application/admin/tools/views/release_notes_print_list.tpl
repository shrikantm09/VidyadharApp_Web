...
<%/if%>
<%assign "id" []%>
<%foreach from=$data key=k item=j%>
    <%$id.$k = $j['iReleaseNotesId']%>
<%/foreach%>
<%assign var="release_notes" value=$this->release_notes_model->printReleaseNotesList($id)%>
<%assign var="new_id" value=$smarty.get.id%>
<!-- Print Listing Block -->
<div id="print_this_content" class="print-view-container">
    ...
    <div class="heading" id="top_heading_fix">
        <!-- Top Strip -->
        <h3>
            <%$this->lang->line('GENERIC_PRINT_LISTING')%> :: <%$this->lang->line('RELEASE_NOTES_RELEASE_NOTES')%>
        </h3>
        <!-- Print Layouts -->
        <%if $layout_combo|is_array && $layout_combo|@count gt 0%>
            <div class="frm-switch-drop frm-print-drop">
                <%$this->dropdown->display('vSwitchPrint',"vSwitchPrint","style='width:100%;' class='chosen-select' onchange='return switchModulePrintPage(\"<%$mod_enc_url.print_listing%>\",this.value, \"<%$extra_qstr%>\")' ",'','',$layout)%>
            </div>
        <%/if%>
    </div>
                                ...
                                <div class="content print-preview-padding print-listing-block" id="print_container">
                                    <div class="release-notes grid-data-container pad-calc-container" style="margin-right:16px;">
                                        <div class="margin_horizontal release-notes-dis">
                                            <%foreach from=$release_notes key=k item=i%>
                                                <div class="release-note" id="">
                                                    <h2 style="padding-top:10px;"><%$i[0]["version_no"]%><span class="date"><%$i[0]['release_date']%></span></h2>
                                                    <ul class="note_list">
