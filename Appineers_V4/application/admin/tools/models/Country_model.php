 ...
 *
 * @path application\admin\tools\models\Country_model.php
 *
 * @version 4.4
 *
 * @author CIT Dev Team
...
class Country_model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        $this->table_name = "mod_country";
        $this->table_alias = "mc";
        $this->primary_key = "iCountryId";
        $this->primary_alias = "mc_country_id";
        ...
        $this->switchto_fields = array(
            $this->db->protect("mc.vCountry")
        );
        $this->switchto_options = array(
            $this->db->protect("mc.vCountry")." AS country",
        );
    ...
    public function getData($extra_cond = "", $fields = "", $order_by = "", $group_by = "", $limit = "", $join = "No", $having_cond = '', $list = FALSE)
    {
        ...
        elseif ($list == TRUE)
        {
            ...
            $this->db->select("mc.vCountryCodeISO_3 AS mc_country_code_i_s_o_3");
            $this->db->select("(SELECT COUNT(*) AS tot FROM `mod_state` WHERE `iCountryId` = `mc`.`iCountryId`) AS sys_no_of_states", FALSE);
            $this->db->select("mc.eStatus AS mc_status");
        }
        else
        {
            $this->db->select("mc.iCountryId AS iCountryId");
            $this->db->select("mc.iCountryId AS mc_country_id");
            $this->db->select("mc.vCountry AS mc_country");
            $this->db->select("mc.vCountryCode AS mc_country_code");
            $this->db->select("mc.vCountryCodeISO_3 AS mc_country_code_i_s_o_3");
            $this->db->select("mc.tDescription AS mc_description");
            $this->db->select("mc.eStatus AS mc_status");
        }
    ...
    public function getListConfiguration($name = "")
    {
        $list_config = array(
            ...
            "mc_country_code" => array(
                "name" => "mc_country_code",
                "table_name" => "mod_country",
                "table_alias" => "mc",
                "field_name" => "vCountryCode",
                "source_field" => "mc_country_code",
                "display_query" => "mc.vCountryCode",
                "entry_type" => "Table",
                "data_type" => "char",
                "show_in" => "Both",
                "type" => "textbox",
                "align" => "left",
                "label" => "Country Code",
                "lang_code" => "COUNTRY_COUNTRY_CODE",
            ...
            "mc_country_code_i_s_o_3" => array(
                "name" => "mc_country_code_i_s_o_3",
                "table_name" => "mod_country",
                "table_alias" => "mc",
                "field_name" => "vCountryCodeISO_3",
                "source_field" => "mc_country_code_i_s_o_3",
                "display_query" => "mc.vCountryCodeISO_3",
                "entry_type" => "Table",
                "data_type" => "char",
                "show_in" => "Both",
                "type" => "textbox",
                ...
                "lang_code" => "COUNTRY_COUNTRY_CODE_ISO_C453",
            ...
            "sys_no_of_states" => array(
                "name" => "sys_no_of_states",
                "table_name" => "",
                "table_alias" => "",
                "field_name" => "",
                "source_field" => "",
                "display_query" => "SELECT COUNT(*) AS tot FROM `mod_state` WHERE `iCountryId` = `mc`.`iCountryId`",
                "entry_type" => "Custom",
                "data_type" => "",
                "show_in" => "Both",
                "type" => "textbox",
                ...
                "lang_code" => "COUNTRY_NUMBER_OF_STATES",
    ...
    public function getFormConfiguration($name = "")
    {
        $form_config = array(
            ...
            "mc_description" => array(
                ...
                "type" => "textarea",
                "label" => "Description",
                "lang_code" => "COUNTRY_DESCRIPTION",
                "label_lang" => $this->lang->line('COUNTRY_DESCRIPTION')
            ),
            "mc_status" => array(
                "name" => "mc_status",
                "table_name" => "mod_country",
