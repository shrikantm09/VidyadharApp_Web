...
/**
 * Description of Shortcuts Controller
 *
 * @category admin
 *
 * @package tools
 ...
 * @class Shortcuts.php
 *
 * @path application\admin\tools\controllers\Shortcuts.php
 *
 * @version 4.3
 *
 * @author CIT Dev Team
 ...
 * @since 06.03.2019
 */
class Shortcuts extends Cit_Controller
{

    /**
     * __construct method is used to set controller preferences while controller object initialization.
     * @created CIT Admin | 04.03.2019
     * @modified CIT Admin | 06.03.2019
     */
    public function __construct()
    {
        ...
        $this->load->model('shortcuts_model');
        $this->_request_params();
        $this->folder_name = "tools";
        $this->module_name = "shortcuts";
        ...
        $this->dropdown_arr = array(
            "ms_shortcut_type" => array(
                "type" => "enum",
                "default" => "No",
                "values" => array(
                    array(
                        'id' => 'Menu',
                        'val' => $this->lang->line('SHORTCUTS_MENU')
                    ),
                    array(
                        'id' => 'Module',
                        'val' => $this->lang->line('SHORTCUTS_MODULE')
                    ),
                    array(
                        'id' => 'Custom',
                        'val' => $this->lang->line('SHORTCUTS_CUSTOM')
                    ),
                    array(
                        'id' => 'General',
                        'val' => $this->lang->line('SHORTCUTS_GENERAL')
                    )
                )
            ),
            "sys_custom_menu" => array(
                "type" => "phpfn",
                "function" => "controller::getMenuDropdown",
                "default" => "Yes",
            ),
            "sys_custom_module" => array(
                "type" => "phpfn",
                "function" => "controller::getModuleDropdown",
                "default" => "Yes",
            ),
            "sys_custom_general" => array(
                "type" => "phpfn",
                "function" => "controller::getGeneralDropdown",
                "default" => "Yes",
            ),
            "ms_status" => array(
                "type" => "enum",
                ...
                "values" => array(
                    array(
                        'id' => 'Active',
                        'val' => $this->lang->line('SHORTCUTS_ACTIVE')
                    ),
                    array(
                        'id' => 'Inactive',
                        'val' => $this->lang->line('SHORTCUTS_INACTIVE')
                    )
                )
            ),
            "ms_shortcut_mode" => array(
                "type" => "enum",
                "default" => "Yes",
                "values" => array(
                    array(
                        'id' => 'System',
                        'val' => $this->lang->line('SHORTCUTS_SYSTEM')
                    ),
                    array(
                        'id' => 'Default',
                        'val' => $this->lang->line('SHORTCUTS_DEFAULT')
                    ),
                    array(
                        'id' => 'Custom',
                        'val' => $this->lang->line('SHORTCUTS_CUSTOM')
    ...
    public function index()
    {
        ...
        try {
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("shortcuts", $access_list, TRUE, TRUE);
            }
            if (!$list_access) {
                throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_VIEW_THIS_PAGE_C46_C46_C33'));
            }
            $enc_loc_module = $this->general->getMD5EncryptString("ListPrefer", "shortcuts");
            ...
            $status_label = array(
                'js_lang_label.SHORTCUTS_ACTIVE',
                'js_lang_label.SHORTCUTS_INACTIVE',
            );

            $list_config = $this->shortcuts_model->getListConfiguration();
            $this->processConfiguration($list_config, $add_access, $edit_access, TRUE);
            $this->general->trackModuleNavigation("Module", "List", "Viewed", $this->mod_enc_url["index"], "shortcuts");
            ...
            $render_arr = array(
                ...
                'default_filters' => $this->shortcuts_model->default_filters,
            );
            $this->smarty->assign($render_arr);
            $this->loadView("shortcuts_index");
        } catch (Exception $e) {
    ...
    public function listing()
    {
        ...
        $list_config = $this->shortcuts_model->getListConfiguration();
        $form_config = $this->shortcuts_model->getFormConfiguration();
        $extra_cond = $this->shortcuts_model->extra_cond;
        $groupby_cond = $this->shortcuts_model->groupby_cond;
        $having_cond = $this->shortcuts_model->having_cond;
        $orderby_cond = $this->shortcuts_model->orderby_cond;
        ...
        $data_recs = $this->shortcuts_model->getListingData($data_config);
        $data_recs['no_records_msg'] = $this->general->processMessageLabel('ACTION_NO_SHORTCUTS_DATA_FOUND_C46_C46_C33');
    ...
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
            $this->filter->checkAccessCapability("shortcuts_export");
        } else {
            $this->filter->getModuleWiseAccess("shortcuts", "Export", TRUE);
        }
        $params_arr = $this->params_arr;
        $page = $params_arr['page'];
        ...
        $list_config = $this->shortcuts_model->getListConfiguration();
        $form_config = $this->shortcuts_model->getFormConfiguration();
        $table_name = $this->shortcuts_model->table_name;
        $table_alias = $this->shortcuts_modeltable_alias;
        $primary_key = $this->shortcuts_model->primary_key;
        $extra_cond = $this->shortcuts_model->extra_cond;
        $groupby_cond = $this->shortcuts_model->groupby_cond;
        $having_cond = $this->shortcuts_model->having_cond;
        $orderby_cond = $this->shortcuts_model->orderby_cond;
        ...
        $db_recs = $this->shortcuts_model->getExportData($export_config);
        ...
        $misc_info['heading'] = $this->lang->line('SHORTCUTS_SHORTCUTS');
        $misc_info['filename'] = "shortcuts_records_" . count($db_recs);
        ...
        try {
            ...
            if ($mode == "Update") {
                if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("shortcuts", $access_list, TRUE, TRUE);
                }
                ...
                if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("shortcuts", $access_list, TRUE, TRUE);
                }
                if (!$add_access) {
            ...
            if ($mode == 'Update') {
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowEdit", "shortcuts"), $this->session->userdata('iAdminId'));
                $data_arr = $this->shortcuts_model->getData(intval($id));
                ...
                if (isset($this->params_arr['switchIDs']) && $this->params_arr['switchIDs'] != "") {
                    ...
                    $switchCond = $this->db->protect($this->shortcuts_model->table_alias . "." . $this->shortcuts_model->primary_key) . " IN (" . $switchEsc . ")";
                    if ($extra_cond != "") {
                ...
                $switch_arr = $this->shortcuts_model->getSwitchTo($extra_cond, "records", $this->switchto_limit);
                $switch_combo = $this->filter->makeArrayDropDown($switch_arr);
                $switch_cit = array();
                $switch_tot = $this->shortcuts_model->getSwitchTo($extra_cond, "count");
                if ($this->switchto_limit > 0 && $switch_tot > $this->switchto_limit) {
                    ...
                    if (!array_key_exists($id, $switch_combo)) {
                        $extra_cond = $this->db->protect($this->shortcuts_model->table_alias . "." . $this->shortcuts_model->primary_key) . " = " . $this->db->escape($id);
                        $switch_cur = $this->shortcuts_model->getSwitchTo($extra_cond, "records", 1);
                ...
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "shortcuts", $recName);
            } else {
                $recName = '';
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowAdd", "shortcuts"), $this->session->userdata('iAdminId'));
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "shortcuts");
            }

            $opt_arr = $img_html = $auto_arr = $config_arr = array();

            $form_config = $this->shortcuts_model->getFormConfiguration($config_arr);
                ...
                foreach ($form_config as $key => $val) {
                    ...
                    if ($val['function'] != "") {
                        ...
                        if (substr($phpfunc, 0, 12) == 'controller::' && substr($phpfunc, 12) !== FALSE) {
                            ...
                            if (method_exists($this->shortcuts_model, $phpfunc)) {
                                $tmpdata = $this->shortcuts_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                    ...
                    if ($val['field_status'] != "") {
                        ...
                        if ($status_type == "capability" && $fd_callback != "") {
                            ...
                            if (substr($fd_callback, 0, 12) == 'controller::' && substr($fd_callback, 12) !== FALSE) {
                                ...
                                if (method_exists($this->shortcuts_model, $fd_callback)) {
                                    $fd_status = $this->shortcuts_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
            ...
            if (method_exists($this, 'beforeLoadCallback')) {
                $render_arr = $this->beforeLoadCallback($render_arr);
            }
            $this->smarty->assign($render_arr);
            if ($mode == "Update") {
                if ($edit_access && $viewMode != TRUE) {
                    $this->loadView("shortcuts_add");
                } else {
                    $this->loadView("shortcuts_add_view");
                }
            } else {
                $this->loadView("shortcuts_add");
            }
    ...
    public function addAction()
    {
        ...
        try {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
                if ($mode == "Update") {
                    $add_edit_access = $this->filter->checkAccessCapability("shortcuts_update", TRUE);
                } else {
                    $add_edit_access = $this->filter->checkAccessCapability("shortcuts_add", TRUE);
                }
            } else {
                $add_edit_access = $this->filter->getModuleWiseAccess("shortcuts", $mode, TRUE, TRUE);
            }
            if (!$add_edit_access) {
                if ($mode == "Update") {
            ...
            $ms_shortcut_key = $params_arr["ms_shortcut_key"];
            $ms_shortcut_name = $params_arr["ms_shortcut_name"];
            $ms_shortcut_type = $params_arr["ms_shortcut_type"];
            $ms_status = $params_arr["ms_status"];
            $ms_shortcut_mode = $params_arr["ms_shortcut_mode"];
            $ms_navigate_json = $params_arr["ms_navigate_json"];
            $ms_added_date = $params_arr["ms_added_date"];
            $ms_modified_date = $params_arr["ms_modified_date"];

            $unique_arr = array();
            $unique_arr["vShortcutKey"] = $ms_shortcut_key;

            $unique_exists = $this->shortcuts_model->checkRecordExists($this->shortcuts_model->unique_fields, $unique_arr, $id, $mode, $this->shortcuts_model->unique_type);
            if ($unique_exists) {
                $error_msg = $this->general->processMessageLabel('ACTION_RECORD_ALREADY_EXISTS_WITH_THESE_DETAILS_OF_SHORTCUT_KEY_C46_C46_C33');
                if ($error_msg == "") {
                    $error_msg = "Record already exists with these details of Shortcut Key";
                }
                throw new Exception($error_msg);
            }
            $data = $save_data_arr = $file_data = array();
            $data["vShortcutKey"] = $ms_shortcut_key;
            $data["vShortcutName"] = $ms_shortcut_name;
            $data["eShortcutType"] = $ms_shortcut_type;
            $data["eStatus"] = $ms_status;
            $data["eShortcutMode"] = $ms_shortcut_mode;
            $data["tNavigateJSON"] = $ms_navigate_json;
            $data["dAddedDate"] = $this->filter->formatActionData($ms_added_date, $form_config["ms_added_date"]);
            $data["dModifiedDate"] = $this->filter->formatActionData($ms_modified_date, $form_config["ms_modified_date"]);

            $save_data_arr["ms_shortcut_key"] = $data["vShortcutKey"];
            $save_data_arr["ms_shortcut_name"] = $data["vShortcutName"];
            $save_data_arr["ms_shortcut_type"] = $data["eShortcutType"];
            $save_data_arr["ms_status"] = $data["eStatus"];
            $save_data_arr["ms_shortcut_mode"] = $data["eShortcutMode"];
            $save_data_arr["ms_navigate_json"] = $data["tNavigateJSON"];
            $save_data_arr["ms_added_date"] = $data["dAddedDate"];
            $save_data_arr["ms_modified_date"] = $data["dModifiedDate"];
            if ($mode == 'Add') {
                $id = $this->shortcuts_model->insert($data);
                if (intval($id) > 0) {
                    $save_data_arr["iShortcutId"] = $data["iShortcutId"] = $id;
                ...
                $track_cond = $this->db->protect("ms.iShortcutId") . " = " . $this->db->escape($id);
                $switch_combo = $this->shortcuts_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Added", $this->mod_enc_url["add"], "shortcuts", $recName, "mode|" . $this->general->getAdminEncodeURL("Update") . "|id|" . $this->general->getAdminEncodeURL($id));
            } elseif ($mode == 'Update') {
                $res = $this->shortcuts_model->update($data, intval($id));
                if (intval($res) > 0) {
                    $save_data_arr["iShortcutId"] = $data["iShortcutId"] = $id;
                ...
                $switch_combo = $this->shortcuts_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Modified", $this->mod_enc_url["add"], "shortcuts", $recName, "mode|" . $this->general->getAdminEncodeURL("Update") . "|id|" . $this->general->getAdminEncodeURL($id));
            }
            $ret_arr['id'] = $id;
            $ret_arr['mode'] = $mode;
            $ret_arr['message'] = $msg;
            ...
            if (method_exists($this, 'afterSaveCallback')) {
                $event_res = $this->afterSaveCallback($mode, $id, $params_arr['parID'], $params_arr['parMod']);
    ...
    public function inlineEditAction()
    {
        ...
        if ($all_row_selected == "true" && in_array($operartor, array("del", "status"))) {
            ...
            $config_arr['list_config'] = $this->shortcuts_model->getListConfiguration();
            $config_arr['form_config'] = $this->shortcuts_model->getFormConfiguration();
            $config_arr['table_name'] = $this->shortcuts_model->table_name;
            $config_arr['table_alias'] = $this->shortcuts_model->table_alias;
        ...
        if ($search_alias == "Yes") {
            $primary_field = $this->shortcuts_model->table_alias . "." . $this->shortcuts_model->primary_key;
        } else {
            $primary_field = $this->shortcuts_model->primary_key;
        }
        ...
        try {
            switch ($operartor) {
                case 'del':
                    $mode = "Delete";
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
                        $del_access = $this->filter->checkAccessCapability("shortcuts_delete", TRUE);
                    } else {
                        $del_access = $this->filter->getModuleWiseAccess("shortcuts", "Delete", TRUE, TRUE);
                    }
                    if (!$del_access) {
                    ...
                    $success = $this->shortcuts_model->delete($extra_cond, $search_alias, $search_join);
                    if (!$success) {
                        throw new Exception($this->general->processMessageLabel('ACTION_FAILURE_IN_DELETION_THIS_RECORD_C46_C46_C33'));
                    }
                ...
                case 'edit':
                    $mode = "Update";
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
                        $edit_access = $this->filter->checkAccessCapability("shortcuts_update", TRUE);
                    } else {
                        $edit_access = $this->filter->getModuleWiseAccess("shortcuts", "Update", TRUE, TRUE);
                    }
                    if (!$edit_access) {
                        throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_MODIFY_THESE_DETAILS_C46_C46_C33'));
                    }
                    ...
                    $list_config = $this->shortcuts_model->getListConfiguration($post_name);
                    $form_config = $this->shortcuts_model->getFormConfiguration($list_config['source_field']);
                    if (!is_array($form_config) || count($form_config) == 0) {
                    ...
                    if (in_array($field_name, $this->shortcuts_model->unique_fields)) {
                        $unique_exists = $this->shortcuts_model->checkRecordExists($this->shortcuts_model->unique_fields, $unique_arr, $primary_ids, "Update", $this->shortcuts_model->unique_type);
                        if ($unique_exists) {
                            $error_msg = $this->general->processMessageLabel('ACTION_RECORD_ALREADY_EXISTS_WITH_THESE_DETAILS_OF_SHORTCUT_KEY_C46_C46_C33');
                            if ($error_msg == "") {
                                $error_msg = "Record already exists with these details of Shortcut Key";
                    ...
                    $success = $this->shortcuts_model->update($data_arr, intval($primary_ids));
                ...
                case 'status':
                    ...
                    if ($search_mode == "Yes" || $search_alias == "Yes") {
                        $field_name = $this->shortcuts_model->table_alias . ".eStatus";
                    } else {
                        $field_name = $status_field;
                    }
                    $data_arr[$field_name] = $params_arr['status'];
                    $success = $this->shortcuts_model->update($data_arr, $extra_cond, $search_alias, $search_join);
                    if (!$success) {
        ...
        switch ($type) {
            ...
            case 'table':
                ...
                if ($combo_config['parent_child'] == "Yes" && $combo_config['nlevel_child'] == "Yes") {
                    $combo_config['main_table'] = $this->shortcuts_model->table_name;
            ...
            case 'phpfn':
                ...
                if (substr($phpfunc, 0, 12) == 'controller::' && substr($phpfunc, 12) !== FALSE) {
                    ...
                    $phpfunc = substr($phpfunc, 7);
                    if (method_exists($this->shortcuts_model, $phpfunc)) {
                        $data_arr = $this->shortcuts_model->$phpfunc($data[$name], $mode, $id, $data, $parent_src, $this->term);
                    }
                } elseif (method_exists($this->general, $phpfunc)) {
    ...
    public function getSelfSwitchTo()
    {
        ...
        $switchto_fields = $this->shortcuts_model->switchto_fields;
        $extra_cond = $this->shortcuts_model->extra_cond;
        ...
        $switch_arr = $this->shortcuts_model->getSwitchTo($extra_cond);
    ...
    public function getListOptions()
    {
        ...
        $config_arr = $this->shortcuts_model->getListConfiguration($alias_name);
    ...
    {
        $menu_arr = $this->systemsettings->getMenuArray("Sitemap");
        foreach ($menu_arr['menu'] as $key => $val) {
            foreach ($val as $key1 => $val1) {
                $menu[$val1['id']]['label'] = $val1['label'];
                $menu[$val1['id']]['url'] = $val1['url'];
                $menu[$val1['id']]['id'] = $val1['id'];
                $menu[$val1['id']]['val'] = $val1['label'];
            }
        }
        ...
        foreach ($menu_arr['menu'] as $key => $val) {
            foreach ($val as $key1 => $val1) {
                if ($val1['parent_id'] != '') {
                    $menu[$val1['parent_id']][$val1['id']]['label'] = $val1['label'];
                    $menu[$val1['parent_id']][$val1['id']]['parent_id'] = $val1['parent_id'];
                    $menu[$val1['parent_id']][$val1['id']]['url'] = $val1['url'];
                    $menu[$val1['parent_id']][$val1['id']]['icon'] = $val1['icon'];
                } else {
                    $type[$val1['id']]['cat_id'] = $val1['label'];
                }
            }
        }
        foreach ($menu as $key => $val) {
            foreach ($val as $key1 => $val1) {
                $list[$key1]['type'] = $type[$val1['parent_id']]['cat_id'];
                $list[$key1]['label'] = $val1['label'];
                $list[$key1]['url'] = $val1['url'];
                $list[$key1]['icon'] = $val1['icon'];
            }
        }

        $list = array_values($list);
        $searchword = $this->input->get_post('request', TRUE);
        foreach ($list as $val) {
            if (stripos((string) $val['label'], $searchword) !== false) {
                $matches[] = $val;
            }
        }
        if (count($matches) == 0) {
            $no_data['type'] = "";
            $no_data['label'] = "No Module found";
            $no_data['url'] = "0";
            $no_data['icon'] = "icomoon-icon-error";
            $matches[] = $no_data;
        }
        $ret_arr['data'] = $matches;
    ...
    {
        $fields_str = 'vShortcutKey as key, vShortcutName as name, eShortcutType as type, eStatus as status';
        $shortcuts = $this->shortcuts_model->getData('', $fields_str);
        $shortcuts1 = $shortcuts2 = $shortcuts3 = $shortcuts4 = array();
        foreach ($shortcuts as $key => $val) {
            $key_arr = explode("+", $val['key']);
            $val['split'] = $key_arr;
            if (count($key_arr) == 1) {
                $shortcuts1[] = $val;
        ...
        $shortcuts_arr = array_merge($shortcuts1, $shortcuts2, $shortcuts3, $shortcuts4);
        $share = ceil(count($shortcuts_arr) / 2);
        $i = 1;
        foreach ($shortcuts_arr as $key => $val) {
            if ($i <= $share) {
                $shortcuts_arr1[] = $val;
            } else {
                $shortcuts_arr2[] = $val;
        ...
        $access_url = $this->general->getCustomEncryptURL('settings_index', TRUE);
        $data_arr['shortcuts_access'] = $this->config->item('ADMIN_SHORTCUT_ACTIVATE');
        $data_arr['shortcuts_arr1'] = $shortcuts_arr1;
        $data_arr['shortcuts_arr2'] = $shortcuts_arr2;
        $data_arr['shortcuts_activate_link'] = $this->config->item("admin_url") . '#' .$access_url['settings_index'] . '|mode|' . $this->general->getAdminEncodeURL('Preferences');
        
        $this->loadView("shortcuts", $data_arr);
    }

    public function getMenuDropdown()
    {
        $menu_arr = $this->getMenuData();
        $i = 0;
        foreach ($menu_arr as $key => $val) {
            if ($val['iParentId'] != 0) {
                $menu[$i]['id'] = $val['iAdminMenuId'];
                $menu[$i]['val'] = $val['vMenuDisplay'];
                $i++;
            }
        }
    ...
    {
        $module_arr = $this->config->load('cit_modules', TRUE);
        $i = 0;
        foreach ($module_arr as $key => $val) {
            $module[$i]['id'] = $key . "_list";
            $module[$i]['val'] = $val['title'] . " - List";
            $i++;
            $module[$i]['id'] = $key . "_add";
            $module[$i]['val'] = $val['title'] . " - Add";
            $i++;
        }
        return $module;
    }

    public function getGeneralDropdown()
    {
        $fields = array("vShortcutName", "tNavigateJSON", "vShortcutKey");
        $this->db->select($fields);
        $this->db->where('eShortcutType', 'General');
        $this->db->where('eShortcutMode', 'Default');
        $this->db->where('eStatus', 'Active');
        $this->db->order_by('iShortcutId', 'ASC');
        $result_obj = $this->db->get('mod_shortcuts');
        $result_arr = is_object($result_obj) ? $result_obj->result_array() : array();
        $i = 0;
        foreach ($result_arr as $key => $val) {
            $json_data = json_decode($val['tNavigateJSON'], true);
            $gen[$i]['id'] = $json_data[$val['vShortcutKey']]['action_code'];
            $gen[$i]['val'] = $val['vShortcutName'];
            $i++;
        }
        return $gen;
    }
    ...
    {
        $fields = array("iAdminMenuId", "vMenuDisplay", "vURL", "iParentId");
        $this->db->select($fields);
        if ($id != '') {
    ...
    public function afterSaveCallback($mode = '', $id = '', $parID = '')
    {
        $postData = $this->input->post();
        $key = $postData['ms_shortcut_key'];
        $name = $postData['ms_shortcut_name'];
        $type = $postData['ms_shortcut_type'];
        $menu_id = $postData['sys_custom_menu'];
        $module_id = $postData['sys_custom_module'];
        $custom = $postData['sys_custom_code'];
        $general_id = $postData['sys_custom_general'];

        if ($type == 'Menu') {
            $menu_arr = $this->getMenuData($menu_id);
            $nav = array($key => array(
                    "menu_url" => $menu_arr[0]['vURL'],
                    "menu_id" => $menu_arr[0]['iAdminMenuId'],
                    "menu_name" => $menu_arr[0]['vMenuDisplay']
            ));
        } else if ($type == 'Module') {
            $module_arr = $this->config->load('cit_modules', TRUE);

            if (substr($module_id, -3) == 'add') {
                $type = 'add';
                $url = 'add_url';
                $module_name = substr($module_id, 0, -4);
            } else if (substr($module_id, -4) == 'list') {
                $type = 'list';
                $url = 'listing_url';
                $module_name = substr($module_id, 0, -5);
            }

            $module = '';
            $nav = array($key => array(
                    "module_url" => $module_arr[$module_name][$url],
                    "module_name" => $module_name,
                    "module_title" => $module_arr[$module_name]['title'],
                    "module_type" => $type
            ));
        } else if ($type == 'General') {
            $nav = array($key => array(
                    "action_code" => $general_id,
            ));
        } else if ($type == 'Custom') {
            $nav = array($key => array(
        ...
        $tNavigateJSON = json_encode($nav);

        $data = array(
            'tNavigateJSON' => $tNavigateJSON
        );
        $this->db->where('iShortcutId', $id);
        $this->db->update('mod_shortcuts', $data);
        $ret_arr['success'] = true;
        return $ret_arr;
    }

    public function beforeLoadCallback($render_arr)
    {

        $new_data = json_decode($render_arr['data']['ms_navigate_json'], true);
        if ($render_arr['data']['ms_shortcut_type'] == 'Custom') {
            $render_arr['data']['sys_custom_code'] = $new_data[$render_arr['data']['ms_shortcut_key']]['function_name'];
        } elseif ($render_arr['data']['ms_shortcut_type'] == 'Menu') {
            $render_arr['data']['sys_custom_menu'] = $new_data[$render_arr['data']['ms_shortcut_key']]['menu_id'];
        } elseif ($render_arr['data']['ms_shortcut_type'] == 'Module') {
            $menu_name = $new_data[$render_arr['data']['ms_shortcut_key']]['module_name'];
            $menu_type = $new_data[$render_arr['data']['ms_shortcut_key']]['module_type'];
            $render_arr['data']['sys_custom_module'] = $menu_name . "_" . $menu_type;
        } elseif ($render_arr['data']['ms_shortcut_type'] == 'General') {
            $render_arr['data']['sys_custom_general'] = $new_data[$render_arr['data']['ms_shortcut_key']]['action_code'];
        }
