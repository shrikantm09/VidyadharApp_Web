...
class Staticpages extends Cit_Controller
{
    ...
    public function index()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("staticpages", $access_list, TRUE, TRUE);
            }
            if (!$list_access)
            {
    ...
    public function export()
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
        {
            $this->filter->checkAccessCapability("staticpages_export");
        }
        else
        {
        ...
        $misc_info['heading'] = $this->lang->line('STATICPAGES_STATIC_PAGES');
        $misc_info['filename'] = "static_pages_records_".count($db_recs);
        ...
        if ($export_type == 'pdf')
        {
            ...
            {
                $pdf->setModule("staticpages_model");
            }
        ...
        try
        {
            ...
            if ($mode == "Update")
            {
                ...
                else
                {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("staticpages", $access_list, TRUE, TRUE);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("staticpages", $access_list, TRUE, TRUE);
                }
                if (!$add_access)
                {
            ...
            if ($mode == 'Update')
            {
                ...
                if (isset($this->params_arr['switchIDs']) && $this->params_arr['switchIDs'] != "")
                {
                    ...
                    $switchCond = $this->db->protect($this->staticpages_model->table_alias.".".$this->staticpages_model->primary_key)." IN (".$switchEsc.")";
                    if ($extra_cond != "")
                    {
                ...
                foreach ($form_config as $key => $val)
                {
                    ...
                    if ($val['function'] != "")
                    {
                        ...
                        elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                        {
                            ...
                            {
                                $tmpdata = $this->staticpages_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                        }
                    ...
                    if ($val['field_status'] != "")
                    {
                        ...
                        elseif ($status_type == "function")
                        {
                            ...
                            elseif (substr($fd_callback, 0, 7) == 'model::' && substr($fd_callback, 7) !== FALSE)
                            {
                                ...
                                if (method_exists($this->staticpages_model, $fd_callback))
                                {
                                    $fd_status = $this->staticpages_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
                            }
    ...
    public function addAction()
    {
        ...
        try
        {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                if ($mode == "Update")
                {
                    $add_edit_access = $this->filter->checkAccessCapability("staticpages_update", TRUE);
                }
                else
                {
                    $add_edit_access = $this->filter->checkAccessCapability("staticpages_add", TRUE);
                }
            }
            else
            {
            ...
            $mps_version = $params_arr["mps_version"];
            ...
            $save_data_arr["mps_version"] = $data["vVersion"];
        ...
        try
        {
            switch ($operartor)
            {
                case 'del':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $del_access = $this->filter->checkAccessCapability("staticpages_delete", TRUE);
                    }
                    else
                    {
                ...
                case 'edit':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $edit_access = $this->filter->checkAccessCapability("staticpages_update", TRUE);
                    }
                    else
                    {
