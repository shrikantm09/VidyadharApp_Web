...
/**
 * Description of Mobile Applications Controller
 *
 * @category admin
 *
 * @package tools
 ...
 *
 * @class Mobile_applications.php
 *
 * @path application\admin\tools\controllers\Mobile_applications.php
 *
 * @version 4.3
 *
 * @author CIT Dev Team
...
class Mobile_applications extends Cit_Controller
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->load->model('mobile_applications_model');
        $this->_request_params();
        $this->folder_name = "tools";
        $this->module_name = "mobile_applications";
        ...
        $this->dropdown_arr = array(
            "mam_device_type" => array(
                "type" => "enum",
                "default" => "Yes",
                "values" => array(
                    array(
                        'id' => 'IOS',
                        'val' => $this->lang->line('MOBILE_APPLICATIONS_IOS')
                    ),
                    array(
                        'id' => 'Android',
                        'val' => $this->lang->line('MOBILE_APPLICATIONS_ANDROID')
                    )
                )
            ),
            "mam_status" => array(
                ...
                "values" => array(
                    array(
                        'id' => 'Active',
                        'val' => $this->lang->line('MOBILE_APPLICATIONS_ACTIVE')
                    ),
                    array(
                        'id' => 'Inactive',
                        'val' => $this->lang->line('MOBILE_APPLICATIONS_INACTIVE')
                    )
                )
            ),
            "mam_added_by" => array(
    ...
    public function index()
    {
        ...
        try
        {
            ...
            {
                $access_list = array(
                    "mobile_applications_list",
                    "mobile_applications_view",
                    "mobile_applications_add",
                    "mobile_applications_update",
                    "mobile_applications_delete",
                    "mobile_applications_export",
                    "mobile_applications_print",
                );
            ...
            else
            {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("mobile_applications", $access_list, TRUE, TRUE);
            }
            if (!$list_access)
            {
                throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_VIEW_THIS_PAGE_C46_C46_C33'));
            }
            $enc_loc_module = $this->general->getMD5EncryptString("ListPrefer", "mobile_applications");
            ...
            $status_label = array(
                'js_lang_label.MOBILE_APPLICATIONS_ACTIVE',
                'js_lang_label.MOBILE_APPLICATIONS_INACTIVE',
            );

            $list_config = $this->mobile_applications_model->getListConfiguration();
            $this->processConfiguration($list_config, $add_access, $edit_access, TRUE);
            $this->general->trackModuleNavigation("Module", "List", "Viewed", $this->mod_enc_url["index"], "mobile_applications");
            ...
            $render_arr = array(
                ...
                'default_filters' => $this->mobile_applications_model->default_filters,
            );
            $this->smarty->assign($render_arr);
            $this->loadView("mobile_applications_index");
        }
        catch(Exception $e)
        {
    ...
    public function listing()
    {
        ...
        $list_config = $this->mobile_applications_model->getListConfiguration();
        $form_config = $this->mobile_applications_model->getFormConfiguration();
        $extra_cond = $this->mobile_applications_model->extra_cond;
        $groupby_cond = $this->mobile_applications_model->groupby_cond;
        $having_cond = $this->mobile_applications_model->having_cond;
        $orderby_cond = $this->mobile_applications_model->orderby_cond;
        ...
        $data_recs = $this->mobile_applications_model->getListingData($data_config);
        $data_recs['no_records_msg'] = $this->general->processMessageLabel('ACTION_NO_MOBILE_APPLICATIONS_DATA_FOUND_C46_C46_C33');
    ...
    public function export()
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
        {
            $this->filter->checkAccessCapability("mobile_applications_export");
        }
        else
        {
            $this->filter->getModuleWiseAccess("mobile_applications", "Export", TRUE);
        }
        $params_arr = $this->params_arr;
        $page = $params_arr['page'];
        ...
        $list_config = $this->mobile_applications_model->getListConfiguration();
        $form_config = $this->mobile_applications_model->getFormConfiguration();
        $table_name = $this->mobile_applications_model->table_name;
        $table_alias = $this->mobile_applications_modeltable_alias;
        $primary_key = $this->mobile_applications_model->primary_key;
        $extra_cond = $this->mobile_applications_model->extra_cond;
        $groupby_cond = $this->mobile_applications_model->groupby_cond;
        $having_cond = $this->mobile_applications_model->having_cond;
        $orderby_cond = $this->mobile_applications_model->orderby_cond;
        ...
        $db_recs = $this->mobile_applications_model->getExportData($export_config);
        ...
        $misc_info['heading'] = $this->lang->line('MOBILE_APPLICATIONS_MOBILE_APPLICATIONS');
        $misc_info['filename'] = "mobile_applications_records_".count($db_recs);
        ...
        if ($export_type == 'pdf')
        {
            ...
            {
                $pdf->setModule("mobile_applications_model");
            }
        ...
        try
        {
            ...
            if ($mode == "Update")
            {
                ...
                else
                {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("mobile_applications", $access_list, TRUE, TRUE);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("mobile_applications", $access_list, TRUE, TRUE);
                }
                if (!$add_access)
                {
            ...
            if ($mode == 'Update')
            {
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowEdit", "mobile_applications"), $this->session->userdata('iAdminId'));
                $data_arr = $this->mobile_applications_model->getData(intval($id));
                ...
                if ($this->switchto_limit > 0 && $switch_tot > $this->switchto_limit)
                {
                    ...
                    {
                        $extra_cond = $this->db->protect($this->mobile_applications_model->table_alias.".".$this->mobile_applications_model->primary_key)." = ".$this->db->escape($id);
                ...
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "mobile_applications", $recName);
            }
            else
            {
                $recName = '';
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowAdd", "mobile_applications"), $this->session->userdata('iAdminId'));
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "mobile_applications");
            }

            $opt_arr = $img_html = $auto_arr = $config_arr = array();

            $form_config = $this->mobile_applications_model->getFormConfiguration($config_arr);
                ...
                foreach ($form_config as $key => $val)
                {
                    ...
                    if ($val['function'] != "")
                    {
                        ...
                        elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                        {
                            ...
                            if (method_exists($this->mobile_applications_model, $phpfunc))
                            {
                                $tmpdata = $this->mobile_applications_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                        }
                    ...
                    if ($val['field_status'] != "")
                    {
                        ...
                        elseif ($status_type == "function")
                        {
                            ...
                            elseif (substr($fd_callback, 0, 7) == 'model::' && substr($fd_callback, 7) !== FALSE)
                            {
                                ...
                                if (method_exists($this->mobile_applications_model, $fd_callback))
                                {
                                    $fd_status = $this->mobile_applications_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
                            }
            ...
            if ($mode == "Update")
            {
                ...
                {
                    $this->loadView("mobile_applications_add");
                }
                else
                {
                    $this->loadView("mobile_applications_add_view");
                }
            }
            else
            {
    ...
    public function addAction()
    {
        ...
        try
        {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                if ($mode == "Update")
                {
                    $add_edit_access = $this->filter->checkAccessCapability("mobile_applications_update", TRUE);
                }
                else
                {
                    $add_edit_access = $this->filter->checkAccessCapability("mobile_applications_add", TRUE);
                }
            }
            else
            {
                $add_edit_access = $this->filter->getModuleWiseAccess("mobile_applications", $mode, TRUE, TRUE);
            }
            if (!$add_edit_access)
            {
                if ($mode == "Update")
            ...
            $mam_application_name = $params_arr["mam_application_name"];
            $mam_device_type = $params_arr["mam_device_type"];
            $mam_status = $params_arr["mam_status"];
            $mam_date_added = $params_arr["mam_date_added"];
            $mam_added_by = $params_arr["mam_added_by"];
            $mam_date_updated = $params_arr["mam_date_updated"];
            $mam_updated_by = $params_arr["mam_updated_by"];

            $unique_arr = array();
            $unique_arr["vApplicationName"] = $mam_application_name;

            $unique_exists = $this->mobile_applications_model->checkRecordExists($this->mobile_applications_model->unique_fields, $unique_arr, $id, $mode, $this->mobile_applications_model->unique_type);
            if ($unique_exists)
            {
                $error_msg = $this->general->processMessageLabel('ACTION_RECORD_ALREADY_EXISTS_WITH_THESE_DETAILS_OF_APPLICATION_NAME_C46_C46_C33');
                if ($error_msg == "")
                {
                    $error_msg = "Record already exists with these details of Application Name";
                }
                throw new Exception($error_msg);
            }
            $data = $save_data_arr = $file_data = array();
            $data["vApplicationName"] = $mam_application_name;
            $data["eDeviceType"] = $mam_device_type;
            $data["eStatus"] = $mam_status;
            $data["dDateAdded"] = $this->filter->formatActionData($mam_date_added, $form_config["mam_date_added"]);
            $data["iAddedBy"] = $mam_added_by;
            $data["dDateUpdated"] = $this->filter->formatActionData($mam_date_updated, $form_config["mam_date_updated"]);
            $data["iUpdatedBy"] = $mam_updated_by;

            $save_data_arr["mam_application_name"] = $data["vApplicationName"];
            $save_data_arr["mam_device_type"] = $data["eDeviceType"];
            $save_data_arr["mam_status"] = $data["eStatus"];
            $save_data_arr["mam_date_added"] = $data["dDateAdded"];
            $save_data_arr["mam_added_by"] = $data["iAddedBy"];
            $save_data_arr["mam_date_updated"] = $data["dDateUpdated"];
            $save_data_arr["mam_updated_by"] = $data["iUpdatedBy"];
            if ($mode == 'Add')
            {
                $id = $this->mobile_applications_model->insert($data);
                if (intval($id) > 0)
                {
                    $save_data_arr["iApplicationMasterId"] = $data["iApplicationMasterId"] = $id;
                ...
                $track_cond = $this->db->protect("mam.iApplicationMasterId")." = ".$this->db->escape($id);
                $switch_combo = $this->mobile_applications_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Added", $this->mod_enc_url["add"], "mobile_applications", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            elseif ($mode == 'Update')
            {
                $res = $this->mobile_applications_model->update($data, intval($id));
                if (intval($res) > 0)
                {
                    $save_data_arr["iApplicationMasterId"] = $data["iApplicationMasterId"] = $id;
                ...
                $switch_combo = $this->mobile_applications_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Modified", $this->mod_enc_url["add"], "mobile_applications", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            $ret_arr['id'] = $id;
            $ret_arr['mode'] = $mode;
            $ret_arr['message'] = $msg;
    ...
    public function inlineEditAction()
    {
        ...
        if ($all_row_selected == "true" && in_array($operartor, array("del", "status")))
        {
            ...
            $config_arr['list_config'] = $this->mobile_applications_model->getListConfiguration();
            $config_arr['form_config'] = $this->mobile_applications_model->getFormConfiguration();
            $config_arr['table_name'] = $this->mobile_applications_model->table_name;
            $config_arr['table_alias'] = $this->mobile_applications_model->table_alias;
        ...
        if ($search_alias == "Yes")
        {
            $primary_field = $this->mobile_applications_model->table_alias.".".$this->mobile_applications_model->primary_key;
        }
        else
        {
            $primary_field = $this->mobile_applications_model->primary_key;
        }
        ...
        try
        {
            switch ($operartor)
            {
                case 'del':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $del_access = $this->filter->checkAccessCapability("mobile_applications_delete", TRUE);
                    }
                    else
                    {
                        $del_access = $this->filter->getModuleWiseAccess("mobile_applications", "Delete", TRUE, TRUE);
                    }
                    if (!$del_access)
                    {
                    ...
                    $success = $this->mobile_applications_model->delete($extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_FAILURE_IN_DELETION_THIS_RECORD_C46_C46_C33'));
                    }
                ...
                case 'edit':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $edit_access = $this->filter->checkAccessCapability("mobile_applications_update", TRUE);
                    }
                    else
                    {
                        $edit_access = $this->filter->getModuleWiseAccess("mobile_applications", "Update", TRUE, TRUE);
                    }
                    if (!$edit_access)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_MODIFY_THESE_DETAILS_C46_C46_C33'));
                    ...
                    $list_config = $this->mobile_applications_model->getListConfiguration($post_name);
                    $form_config = $this->mobile_applications_model->getFormConfiguration($list_config['source_field']);
                    if (!is_array($form_config) || count($form_config) == 0)
                    {
                    ...
                    if (in_array($field_name, $this->mobile_applications_model->unique_fields))
                    {
                        $unique_exists = $this->mobile_applications_model->checkRecordExists($this->mobile_applications_model->unique_fields, $unique_arr, $primary_ids, "Update", $this->mobile_applications_model->unique_type);
                        if ($unique_exists)
                        {
                            $error_msg = $this->general->processMessageLabel('ACTION_RECORD_ALREADY_EXISTS_WITH_THESE_DETAILS_OF_APPLICATION_NAME_C46_C46_C33');
                            if ($error_msg == "")
                    ...
                    $success = $this->mobile_applications_model->update($data_arr, intval($primary_ids));
                ...
                case 'status':
                    ...
                    {
                        $field_name = $this->mobile_applications_model->table_alias.".eStatus";
                    }
                    else
                    {
                    ...
                    $success = $this->mobile_applications_model->update($data_arr, $extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
    ...
    public function getSourceOptions($name = '', $mode = 'Add', $id = '', $data = array(), $extra = '', $rtype = 'records')
    {
        ...
        switch ($type)
        {
            ...
            case 'table':
                ...
                {
                    $combo_config['main_table'] = $this->mobile_applications_model->table_name;
            ...
            case 'phpfn':
                ...
                elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                {
                    ...
                    if (method_exists($this->mobile_applications_model, $phpfunc))
                    {
                        $data_arr = $this->mobile_applications_model->$phpfunc($data[$name], $mode, $id, $data, $parent_src, $this->term);
                    }
                }
                elseif (method_exists($this->general, $phpfunc))
                {
    ...
    public function getSelfSwitchTo()
    {
        ...
        $switch_arr = $this->mobile_applications_model->getSwitchTo($extra_cond);
    ...
    public function getListOptions()
    {
        ...
        $config_arr = $this->mobile_applications_model->getListConfiguration($alias_name);
    ...
    public function release()
    {
        try {
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
                $list_access = $this->filter->checkAccessCapability("mobile_applications_list", TRUE);
            } else {
                $list_access = $this->filter->getModuleWiseAccess("mobile_applications", "List", TRUE, TRUE);
            }
            if (!$list_access) {
                throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_VIEW_THIS_PAGE_C46_C46_C33'));
            }

            $applications = $this->mobile_applications_model->getApplicationDetails();
            $versions = $this->mobile_applications_model->getVersionsDetails();

            if (is_array($applications) && count($applications) > 0 && is_array($versions) && count($versions) > 0) {
                foreach ($applications as $key => $value) {
                    foreach ($versions as $item) {
                        if ($item['mav_application_master_id'] == $value['mam_application_master_id']) {
                            $applications[$key]['versions_list'][] = $item;
                        }
                    }
                }
            }
            ...
            $render_data['applications'] = $applications;
            $this->smarty->assign($render_data);
            $this->loadview('mobile_applications_release');
        } catch (Exception $e) {
            $render_arr['err_message'] = $e->getMessage();
            $this->smarty->assign($render_arr);
            $this->loadView($this->config->item('ADMIN_FORBIDDEN_TEMPLATE'));
