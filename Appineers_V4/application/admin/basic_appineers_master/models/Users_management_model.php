    ...
    public function update($data = array(), $where = '', $alias = "No", $join = "No")
    {
        ...
        if ($this->db->affected_rows() == 1) {
            ...
            if($where != "")
            {
                ...
                $updatedEntity = $data_arr12[0]['u_email'];

            }
            
            $updatedEntity_log = "user- ".$updatedEntity;

            $this->general->dbChanageLog($this->table_name, $this->primary_key, $this->db->affected_rows(), $data, $where,$updatedEntity_log, "Modified");
        }
                    ...
                    if($whereV != "")
                    {
                        ...
                        $this->db->from($this->table_name . " AS " . $this->table_alias);
                         if (is_numeric($whereV)) {
                                ...
                                $this->db->where($whereV, FALSE, FALSE);
                            } 
            ...
            foreach ($where_values as $key => $whereV) 
            {
               ...
               // $logArray['dDateAdded'] = CURRENT_TIMESTAMP();
                ...
                if($whereV != "")
                {
                    ...
                    $data_obj = $this->db->get();
                    $data_arr12 = is_object($data_obj) ? $data_obj->result_array() : array();
                    $updatedEntity = $data_arr12[0]['vEmail'];

                }
                
                $updatedEntity_log = "user- ".$updatedEntity;

                $this->general->dbChanageLog($this->table_name, $this->primary_key, $this->db->affected_rows(), $data, $where,$updatedEntity_log, "Deleted");
    ...
    public function getData($extra_cond = "", $fields = "", $order_by = "", $group_by = "", $limit = "", $join = "No", $having_cond = '', $list = FALSE)
    {
        ...
        $this->db->select("u.dtUpdatedAt AS u_updated_at");
           ...
           // $this->db->select("u.dDob AS u_dob");
            ...
            $this->db->select("u.eModeratorFlag AS u_is_moderatorflag"); 
        }
    ...
    public function getFormConfiguration($name = "")
    {
        ...
        $form_config = array(
            ...
            "u_is_moderatorflag" => array(
                "name" => "u_is_moderatorflag",
                "table_name" => "users",
                "table_alias" => "u",
                "field_name" => "eModeratorFlag",
                "entry_type" => "Table",
                "data_type" => "enum",
                "show_input" => "Both",
                "type" => "dropdown",
                "label" => "Log Status",
                "lang_code" => "USERS_MANAGEMENT_MODERATOR_STATUS",
                "label_lang" => $this->lang->line('USERS_MANAGEMENT_MODERATOR_STATUS')
            )
        );
