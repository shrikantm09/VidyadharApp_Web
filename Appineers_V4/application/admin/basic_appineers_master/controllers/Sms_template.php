...
/**
 * Description of System SMS Controller
 *
 * @category admin
 *
 ...
 * @module System SMS
 *
 * @class Sms_template.php
 *
 * @path application\admin\basic_appineers_master\controllers\Sms_template.php
 *
 * @version 4.4
 *
 * @author CIT Dev Team
...
class Sms_template extends Cit_Controller
{
    /**
        ...
        $this->dropdown_arr = array(
            "st_status" => array(
                ...
                "values" => array(
                    array(
                        'id' => 'Active',
                        'val' => $this->lang->line('SMS_TEMPLATE_ACTIVE')
                    ),
                    array(
                        'id' => 'Inactive',
                        'val' => $this->lang->line('SMS_TEMPLATE_INACTIVE')
                    )
                )
            )
        );
    ...
    public function index()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("sms_template", $access_list, TRUE, TRUE);
            }
            if (!$list_access)
            {
                throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_VIEW_THIS_PAGE_C46_C46_C33'));
            }
            $enc_loc_module = $this->general->getMD5EncryptString("ListPrefer", "sms_template");
            ...
            $status_label = array(
                'js_lang_label.SMS_TEMPLATE_ACTIVE',
                'js_lang_label.SMS_TEMPLATE_INACTIVE',
            );

            $list_config = $this->sms_template_model->getListConfiguration();
            $this->processConfiguration($list_config, $add_access, $edit_access, TRUE);
            $this->general->trackModuleNavigation("Module", "List", "Viewed", $this->mod_enc_url["index"], "sms_template");
            ...
            $render_arr = array(
                ...
                'default_filters' => $this->sms_template_model->default_filters,
            );
            ...
            {
                $this->loadView("sms_template_index");
            }
        }
        catch(Exception $e)
        {
    ...
    public function listing()
    {
        ...
        $list_config = $this->sms_template_model->getListConfiguration();
        $form_config = $this->sms_template_model->getFormConfiguration();
        $extra_cond = $this->sms_template_model->extra_cond;
        $groupby_cond = $this->sms_template_model->groupby_cond;
        $having_cond = $this->sms_template_model->having_cond;
        $orderby_cond = $this->sms_template_model->orderby_cond;
        ...
        $data_recs = $this->sms_template_model->getListingData($data_config);
        $data_recs['no_records_msg'] = $this->general->processMessageLabel('ACTION_NO_SYSTEM_SMS_DATA_FOUND_C46_C46_C33');
    ...
    public function export()
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
        {
            $this->filter->checkAccessCapability("sms_template_export");
        }
        else
        {
            $this->filter->getModuleWiseAccess("sms_template", "Export", TRUE);
        }
        $params_arr = $this->params_arr;
        $page = $params_arr['page'];
        ...
        $list_config = $this->sms_template_model->getListConfiguration();
        $form_config = $this->sms_template_model->getFormConfiguration();
        $table_name = $this->sms_template_model->table_name;
        $table_alias = $this->sms_template_modeltable_alias;
        $primary_key = $this->sms_template_model->primary_key;
        $extra_cond = $this->sms_template_model->extra_cond;
        $groupby_cond = $this->sms_template_model->groupby_cond;
        $having_cond = $this->sms_template_model->having_cond;
        $orderby_cond = $this->sms_template_model->orderby_cond;
        ...
        $db_recs = $this->sms_template_model->getExportData($export_config);
        ...
        $misc_info['heading'] = $this->lang->line('SMS_TEMPLATE_SYSTEM_SMS');
        $misc_info['filename'] = "system_sms_records_".count($db_recs);
        ...
        if ($export_type == 'pdf')
        {
            ...
            {
                $pdf->setModule("sms_template_model");
            }
        ...
        try
        {
            ...
            if ($mode == "Update")
            {
                ...
                else
                {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("sms_template", $access_list, TRUE, TRUE);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("sms_template", $access_list, TRUE, TRUE);
                }
                if (!$add_access)
                {
            ...
            if ($mode == 'Update')
            {
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowEdit", "sms_template"), $this->session->userdata('iAdminId'));
                $data_arr = $this->sms_template_model->getData(intval($id));
                ...
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "sms_template", $recName);
            }
            else
            {
                $recName = '';
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowAdd", "sms_template"), $this->session->userdata('iAdminId'));
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "sms_template");
            }

            $opt_arr = $img_html = $auto_arr = $config_arr = array();

            $form_config = $this->sms_template_model->getFormConfiguration($config_arr);
                ...
                foreach ($form_config as $key => $val)
                {
                    ...
                    if ($val['function'] != "")
                    {
                        ...
                        elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                        {
                            ...
                            if (method_exists($this->sms_template_model, $phpfunc))
                            {
                                $tmpdata = $this->sms_template_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                        }
                    ...
                    if ($val['field_status'] != "")
                    {
                        ...
                        elseif ($status_type == "function")
                        {
                            ...
                            elseif (substr($fd_callback, 0, 7) == 'model::' && substr($fd_callback, 7) !== FALSE)
                            {
                                ...
                                if (method_exists($this->sms_template_model, $fd_callback))
                                {
                                    $fd_status = $this->sms_template_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
                            }
            ...
            else
            {
                if ($mode == "Update")
                {
                    ...
                    {
                        $this->loadView("sms_template_add");
                    }
                    else
                    {
                        $this->loadView("sms_template_add_view");
                    }
                }
                else
                {
    ...
    public function addAction()
    {
        ...
        try
        {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                if ($mode == "Update")
                {
                    $add_edit_access = $this->filter->checkAccessCapability("sms_template_update", TRUE);
                }
                else
                {
                    $add_edit_access = $this->filter->checkAccessCapability("sms_template_add", TRUE);
                }
            }
            else
            {
                $add_edit_access = $this->filter->getModuleWiseAccess("sms_template", $mode, TRUE, TRUE);
            }
            if (!$add_edit_access)
            {
                if ($mode == "Update")
            ...
            $st_template_code = $params_arr["st_template_code"];
            $st_sms_text = $params_arr["st_sms_text"];
            $st_status = $params_arr["st_status"];
            $st_added_at = $params_arr["st_added_at"];
            $st_updated_at = $params_arr["st_updated_at"];

            $data = $save_data_arr = $file_data = array();
            $data["vTemplateCode"] = $st_template_code;
            $data["tSmsText"] = $st_sms_text;
            $data["eStatus"] = $st_status;
            $data["dtAddedAt"] = $this->filter->formatActionData($st_added_at, $form_config["st_added_at"]);
            $data["dtUpdatedAt"] = $this->filter->formatActionData($st_updated_at, $form_config["st_updated_at"]);

            $save_data_arr["st_template_code"] = $data["vTemplateCode"];
            $save_data_arr["st_sms_text"] = $data["tSmsText"];
            $save_data_arr["st_status"] = $data["eStatus"];
            $save_data_arr["st_added_at"] = $data["dtAddedAt"];
            $save_data_arr["st_updated_at"] = $data["dtUpdatedAt"];
            if ($mode == 'Add')
            {
                $id = $this->sms_template_model->insert($data);
                if (intval($id) > 0)
                {
                    $save_data_arr["iSmsTemplateId"] = $data["iSmsTemplateId"] = $id;
                ...
                $track_cond = $this->db->protect("st.iSmsTemplateId")." = ".$this->db->escape($id);
                $switch_combo = $this->sms_template_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Added", $this->mod_enc_url["add"], "sms_template", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            elseif ($mode == 'Update')
            {
                $res = $this->sms_template_model->update($data, intval($id));
                if (intval($res) > 0)
                {
                    $save_data_arr["iSmsTemplateId"] = $data["iSmsTemplateId"] = $id;
                ...
                $switch_combo = $this->sms_template_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Modified", $this->mod_enc_url["add"], "sms_template", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            $ret_arr['id'] = $id;
            $ret_arr['mode'] = $mode;
            $ret_arr['message'] = $msg;
    ...
    public function inlineEditAction()
    {
        ...
        if ($all_row_selected == "true" && in_array($operartor, array("del", "status")))
        {
            ...
            $config_arr['list_config'] = $this->sms_template_model->getListConfiguration();
            $config_arr['form_config'] = $this->sms_template_model->getFormConfiguration();
            $config_arr['table_name'] = $this->sms_template_model->table_name;
            $config_arr['table_alias'] = $this->sms_template_model->table_alias;
        ...
        if ($search_alias == "Yes")
        {
            $primary_field = $this->sms_template_model->table_alias.".".$this->sms_template_model->primary_key;
        }
        else
        {
            $primary_field = $this->sms_template_model->primary_key;
        }
        ...
        try
        {
            switch ($operartor)
            {
                case 'del':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $del_access = $this->filter->checkAccessCapability("sms_template_delete", TRUE);
                    }
                    else
                    {
                        $del_access = $this->filter->getModuleWiseAccess("sms_template", "Delete", TRUE, TRUE);
                    }
                    if (!$del_access)
                    {
                    ...
                    $success = $this->sms_template_model->delete($extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_FAILURE_IN_DELETION_THIS_RECORD_C46_C46_C33'));
                    }
                ...
                case 'edit':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $edit_access = $this->filter->checkAccessCapability("sms_template_update", TRUE);
                    }
                    else
                    {
                        $edit_access = $this->filter->getModuleWiseAccess("sms_template", "Update", TRUE, TRUE);
                    }
                    if (!$edit_access)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_MODIFY_THESE_DETAILS_C46_C46_C33'));
                    ...
                    $list_config = $this->sms_template_model->getListConfiguration($post_name);
                    $form_config = $this->sms_template_model->getFormConfiguration($list_config['source_field']);
                    if (!is_array($form_config) || count($form_config) == 0)
                    {
                    ...
                    $success = $this->sms_template_model->update($data_arr, intval($primary_ids));
                ...
                case 'status':
                    ...
                    {
                        $field_name = $this->sms_template_model->table_alias.".eStatus";
                    }
                    else
                    {
                    ...
                    $success = $this->sms_template_model->update($data_arr, $extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
    ...
    public function getSourceOptions($name = '', $mode = 'Add', $id = '', $data = array(), $extra = '', $rtype = 'records')
    {
        ...
        switch ($type)
        {
            ...
            case 'table':
                ...
                {
                    $combo_config['main_table'] = $this->sms_template_model->table_name;
            ...
            case 'phpfn':
                ...
                elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                {
                    ...
                    if (method_exists($this->sms_template_model, $phpfunc))
                    {
                        $data_arr = $this->sms_template_model->$phpfunc($data[$name], $mode, $id, $data, $parent_src, $this->term);
                    }
                }
                elseif (method_exists($this->general, $phpfunc))
                {
    ...
    public function getSelfSwitchTo()
    {
        ...
        $switchto_fields = $this->sms_template_model->switchto_fields;
        $extra_cond = $this->sms_template_model->extra_cond;
        ...
        $switch_arr = $this->sms_template_model->getSwitchTo($extra_cond);
    ...
    public function getListOptions()
    {
        ...
        $config_arr = $this->sms_template_model->getListConfiguration($alias_name);
