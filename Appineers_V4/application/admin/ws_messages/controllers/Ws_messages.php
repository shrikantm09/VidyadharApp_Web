...
class Ws_messages extends Cit_Controller
{
        ...
        $this->dropdown_arr = array(
            "mwm_type" => array(
                ...
                "values" => array(
                    ...
                    array(
                        'id' => 'Other',
                        'val' => $this->lang->line('WS_MESSAGES_OTHER')
                    )
                )
            ),
            "mwm_status" => array(
                ...
                "values" => array(
                    ...
                    array(
                        'id' => 'Archive',
                        'val' => $this->lang->line('WS_MESSAGES_ARCHIVE')
                    )
                )
            )
        );
    ...
    public function index()
    {
        ...
        try {
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("ws_messages", $access_list, TRUE, TRUE);
            }
            if (!$list_access) {
            ...
            $status_label = array(
                ...
                'js_lang_label.WS_MESSAGES_ARCHIVE',
            );
    ...
    public function listing()
    {
        ...
        $data_recs['no_records_msg'] = $this->general->processMessageLabel('ACTION_NO_WS_MESSAGES_DATA_FOUND_C46_C46_C33');
    ...
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
            $this->filter->checkAccessCapability("ws_messages_export");
        } else {
        ...
        $misc_info['heading'] = $this->lang->line('WS_MESSAGES_WS_MESSAGES');
        $misc_info['filename'] = "api_messages_records_" . count($db_recs) ;
        ...
        if ($export_type == 'pdf') {
            ...
            if (method_exists($pdf, "setModule")) {
                $pdf->setModule("ws_messages_model");
            }
        ...
        try {
            ...
            if ($mode == "Update") {
                if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("ws_messages", $access_list, TRUE, TRUE);
                }
                ...
                if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("ws_messages", $access_list, TRUE, TRUE);
                }
                if (!$add_access) {
                ...
                foreach ($form_config as $key => $val) {
                    ...
                    if ($val['function'] != "") {
                        ...
                        if (substr($phpfunc, 0, 12) == 'controller::' && substr($phpfunc, 12) !== FALSE) {
                            ...
                            if (method_exists($this->ws_messages_model, $phpfunc)) {
                                $tmpdata = $this->ws_messages_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                    ...
                    if ($val['field_status'] != "") {
                        ...
                        if ($status_type == "capability" && $fd_callback != "") {
                            ...
                            if (substr($fd_callback, 0, 12) == 'controller::' && substr($fd_callback, 12) !== FALSE) {
                                ...
                                if (method_exists($this->ws_messages_model, $fd_callback)) {
                                    $fd_status = $this->ws_messages_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
    ...
    public function addAction()
    {
        ...
        try {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
                if ($mode == "Update") {
                    $add_edit_access = $this->filter->checkAccessCapability("ws_messages_update", TRUE);
                } else {
                    $add_edit_access = $this->filter->checkAccessCapability("ws_messages_add", TRUE);
                }
            } else {
            ...
            $save_data_arr["mwm_code"] = $data["vCode"];
        ...
        try {
            switch ($operartor) {
                case 'del':
                    $mode = "Delete";
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
                        $del_access = $this->filter->checkAccessCapability("ws_messages_delete", TRUE);
                    } else {
                ...
                case 'edit':
                    $mode = "Update";
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES")) {
                        $edit_access = $this->filter->checkAccessCapability("ws_messages_update", TRUE);
                    } else {
        ...
        switch ($type) {
            ...
            case 'phpfn':
                ...
                if (substr($phpfunc, 0, 12) == 'controller::' && substr($phpfunc, 12) !== FALSE) {
                    ...
                    $phpfunc = substr($phpfunc, 7);
                    if (method_exists($this->ws_messages_model, $phpfunc)) {
                        $data_arr = $this->ws_messages_model->$phpfunc($data[$name], $mode, $id, $data, $parent_src, $this->term);
                    }
                } elseif (method_exists($this->general, $phpfunc)) {
