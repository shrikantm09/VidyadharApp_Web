...
class Ws_messages_model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        $this->primary_alias = "mwm_w_smessage_id";
    ...
    public function getData($extra_cond = "", $fields = "", $order_by = "", $group_by = "", $limit = "", $join = "No", $having_cond = '', $list = FALSE)
    {
        if (is_array($fields)) {
            ...
            $this->db->select("mwm.vCode AS mwm_code");
            ...
            $this->db->select("mwm.iWSMessageId AS mwm_w_smessage_id");
            $this->db->select("mwm.vAPIName AS mwm_apiname");
            $this->db->select("mwm.vCode AS mwm_code");
            $this->db->select("mwm.vType AS mwm_type");
            $this->db->select("mwm.eStatus AS mwm_status");
    ...
    public function getListConfiguration($name = "")
    {
        $list_config = array(
            ...
            "mwm_type" => array(
                ...
                "editable" => "No",
                "viewedit" => "No",
                "default" => $this->filter->getDefaultValue("mwm_type", "Text", "Input")
            ),
            "mwm_status" => array(
                "name" => "mwm_status",
                "table_name" => "mod_ws_messages",
                ...
                "editable" => "No",
                "viewedit" => "No",
                "default" => $this->filter->getDefaultValue("mwm_status", "Text", "Active")
            )
        );
