...
/**
 * Description of Log History Controller
 *
 * @category admin
 *
 * @package user
 ...
 *
 * @path application\admin\user\controllers\Loghistory.php
 *
 * @version 4.4
 *
 * @author CIT Dev Team
...
class Loghistory extends Cit_Controller
{
    /**
        ...
        $this->dropdown_arr = array(
            ...
            "mlh_user_type" => array(
                ...
                "values" => array(
                    array(
                        'id' => 'Admin',
                        'val' => $this->lang->line('LOGHISTORY_ADMIN')
                    ),
                    array(
                        'id' => 'Member',
                        'val' => $this->lang->line('LOGHISTORY_MEMBER')
                    )
                )
            )
        );
    ...
    public function index()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("loghistory", $access_list, TRUE, TRUE);
            }
            if (!$list_access)
            {
                throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_VIEW_THIS_PAGE_C46_C46_C33'));
            }
            $enc_loc_module = $this->general->getMD5EncryptString("ListPrefer", "loghistory");
            ...
            $status_label = array(
                'js_lang_label.LOGHISTORY_ADMIN',
                'js_lang_label.LOGHISTORY_MEMBER',
            );

            $list_config = $this->loghistory_model->getListConfiguration();
            $this->processConfiguration($list_config, $add_access, $edit_access, TRUE);
            $this->general->trackModuleNavigation("Module", "List", "Viewed", $this->mod_enc_url["index"], "loghistory");
            ...
            $render_arr = array(
                ...
                'default_filters' => $this->loghistory_model->default_filters,
            );
            ...
            {
                $this->loadView("loghistory_index");
            }
        }
        catch(Exception $e)
        {
    ...
    public function listing()
    {
        ...
        $list_config = $this->loghistory_model->getListConfiguration();
        $form_config = $this->loghistory_model->getFormConfiguration();
        $extra_cond = $this->loghistory_model->extra_cond;
        $groupby_cond = $this->loghistory_model->groupby_cond;
        $having_cond = $this->loghistory_model->having_cond;
        $orderby_cond = $this->loghistory_model->orderby_cond;
        ...
        $data_recs = $this->loghistory_model->getListingData($data_config);
        $data_recs['no_records_msg'] = $this->general->processMessageLabel('ACTION_NO_LOG_HISTORY_DATA_FOUND_C46_C46_C33');
    ...
    public function export()
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
        {
            $this->filter->checkAccessCapability("loghistory_export");
        }
        else
        {
            $this->filter->getModuleWiseAccess("loghistory", "Export", TRUE);
        }
        $params_arr = $this->params_arr;
        $page = $params_arr['page'];
        ...
        $list_config = $this->loghistory_model->getListConfiguration();
        $form_config = $this->loghistory_model->getFormConfiguration();
        $table_name = $this->loghistory_model->table_name;
        $table_alias = $this->loghistory_modeltable_alias;
        $primary_key = $this->loghistory_model->primary_key;
        $extra_cond = $this->loghistory_model->extra_cond;
        $groupby_cond = $this->loghistory_model->groupby_cond;
        $having_cond = $this->loghistory_model->having_cond;
        $orderby_cond = $this->loghistory_model->orderby_cond;
        ...
        $db_recs = $this->loghistory_model->getExportData($export_config);
        ...
        $misc_info['heading'] = $this->lang->line('LOGHISTORY_LOG_HISTORY');
        $misc_info['filename'] = "log_history_records_".count($db_recs);
        ...
        if ($export_type == 'pdf')
        {
            ...
            {
                $pdf->setModule("loghistory_model");
            }
        ...
        try
        {
            ...
            if ($mode == "Update")
            {
                ...
                else
                {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("loghistory", $access_list, TRUE, TRUE);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("loghistory", $access_list, TRUE, TRUE);
                }
                if (!$add_access)
                {
            ...
            if ($mode == 'Update')
            {
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowEdit", "loghistory"), $this->session->userdata('iAdminId'));
                $data_arr = $this->loghistory_model->getData(intval($id));
                ...
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "loghistory", $recName);
            }
            else
            {
                $recName = '';
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowAdd", "loghistory"), $this->session->userdata('iAdminId'));
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "loghistory");
            }

            $opt_arr = $img_html = $auto_arr = $config_arr = array();

            $form_config = $this->loghistory_model->getFormConfiguration($config_arr);
                ...
                foreach ($form_config as $key => $val)
                {
                    ...
                    if ($val['function'] != "")
                    {
                        ...
                        elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                        {
                            ...
                            if (method_exists($this->loghistory_model, $phpfunc))
                            {
                                $tmpdata = $this->loghistory_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                        }
                    ...
                    if ($val['field_status'] != "")
                    {
                        ...
                        elseif ($status_type == "function")
                        {
                            ...
                            elseif (substr($fd_callback, 0, 7) == 'model::' && substr($fd_callback, 7) !== FALSE)
                            {
                                ...
                                if (method_exists($this->loghistory_model, $fd_callback))
                                {
                                    $fd_status = $this->loghistory_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
                            }
            ...
            else
            {
                if ($mode == "Update")
                {
                    ...
                    {
                        $this->loadView("loghistory_add");
                    }
                    else
                    {
                        $this->loadView("loghistory_add_view");
                    }
                }
                else
                {
    ...
    public function addAction()
    {
        ...
        try
        {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                if ($mode == "Update")
                {
                    $add_edit_access = $this->filter->checkAccessCapability("loghistory_update", TRUE);
                }
                else
                {
                    $add_edit_access = $this->filter->checkAccessCapability("loghistory_add", TRUE);
                }
            }
            else
            {
                $add_edit_access = $this->filter->getModuleWiseAccess("loghistory", $mode, TRUE, TRUE);
            }
            if (!$add_edit_access)
            {
                if ($mode == "Update")
            ...
            $mlh_user_id = $params_arr["mlh_user_id"];
            $mlh_i_p = $params_arr["mlh_i_p"];
            $mlh_user_type = $params_arr["mlh_user_type"];
            $mlh_current_url = $params_arr["mlh_current_url"];
            $mlh_extra_param = $params_arr["mlh_extra_param"];
            $mlh_login_date = $params_arr["mlh_login_date"];
            $mlh_logout_date = $params_arr["mlh_logout_date"];

            $data = $save_data_arr = $file_data = array();
            $data["iUserId"] = $mlh_user_id;
            $data["vIP"] = $mlh_i_p;
            $data["eUserType"] = $mlh_user_type;
            $data["vCurrentUrl"] = $mlh_current_url;
            $data["vExtraParam"] = $mlh_extra_param;
            $data["dLoginDate"] = $this->filter->formatActionData($mlh_login_date, $form_config["mlh_login_date"]);
            $data["dLogoutDate"] = $this->filter->formatActionData($mlh_logout_date, $form_config["mlh_logout_date"]);

            $save_data_arr["mlh_user_id"] = $data["iUserId"];
            $save_data_arr["mlh_i_p"] = $data["vIP"];
            $save_data_arr["mlh_user_type"] = $data["eUserType"];
            $save_data_arr["mlh_current_url"] = $data["vCurrentUrl"];
            $save_data_arr["mlh_extra_param"] = $data["vExtraParam"];
            $save_data_arr["mlh_login_date"] = $data["dLoginDate"];
            $save_data_arr["mlh_logout_date"] = $data["dLogoutDate"];
            if ($mode == 'Add')
            {
                $id = $this->loghistory_model->insert($data);
                if (intval($id) > 0)
                {
                    $save_data_arr["iLogId"] = $data["iLogId"] = $id;
                ...
                $track_cond = $this->db->protect("mlh.iLogId")." = ".$this->db->escape($id);
                $switch_combo = $this->loghistory_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Added", $this->mod_enc_url["add"], "loghistory", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            elseif ($mode == 'Update')
            {
                $res = $this->loghistory_model->update($data, intval($id));
                if (intval($res) > 0)
                {
                    $save_data_arr["iLogId"] = $data["iLogId"] = $id;
                ...
                $switch_combo = $this->loghistory_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Modified", $this->mod_enc_url["add"], "loghistory", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            $ret_arr['id'] = $id;
            $ret_arr['mode'] = $mode;
            $ret_arr['message'] = $msg;
    ...
    public function inlineEditAction()
    {
        ...
        if ($all_row_selected == "true" && in_array($operartor, array("del", "status")))
        {
            ...
            $config_arr['list_config'] = $this->loghistory_model->getListConfiguration();
            $config_arr['form_config'] = $this->loghistory_model->getFormConfiguration();
            $config_arr['table_name'] = $this->loghistory_model->table_name;
            $config_arr['table_alias'] = $this->loghistory_model->table_alias;
        ...
        if ($search_alias == "Yes")
        {
            $primary_field = $this->loghistory_model->table_alias.".".$this->loghistory_model->primary_key;
        }
        else
        {
            $primary_field = $this->loghistory_model->primary_key;
        }
        ...
        try
        {
            switch ($operartor)
            {
                case 'del':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $del_access = $this->filter->checkAccessCapability("loghistory_delete", TRUE);
                    }
                    else
                    {
                        $del_access = $this->filter->getModuleWiseAccess("loghistory", "Delete", TRUE, TRUE);
                    }
                    if (!$del_access)
                    {
                    ...
                    $success = $this->loghistory_model->delete($extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_FAILURE_IN_DELETION_THIS_RECORD_C46_C46_C33'));
                    }
                ...
                case 'edit':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $edit_access = $this->filter->checkAccessCapability("loghistory_update", TRUE);
                    }
                    else
                    {
                        $edit_access = $this->filter->getModuleWiseAccess("loghistory", "Update", TRUE, TRUE);
                    }
                    if (!$edit_access)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_MODIFY_THESE_DETAILS_C46_C46_C33'));
                    ...
                    $list_config = $this->loghistory_model->getListConfiguration($post_name);
                    $form_config = $this->loghistory_model->getFormConfiguration($list_config['source_field']);
                    if (!is_array($form_config) || count($form_config) == 0)
                    {
                    ...
                    $success = $this->loghistory_model->update($data_arr, intval($primary_ids));
                ...
                case 'status':
                    ...
                    {
                        $field_name = $this->loghistory_model->table_alias.".eUserType";
                    }
                    else
                    {
                    ...
                    $success = $this->loghistory_model->update($data_arr, $extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
    ...
    public function getSourceOptions($name = '', $mode = 'Add', $id = '', $data = array(), $extra = '', $rtype = 'records')
    {
        ...
        switch ($type)
        {
            ...
            case 'table':
                ...
                {
                    $combo_config['main_table'] = $this->loghistory_model->table_name;
            ...
            case 'phpfn':
                ...
                elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                {
                    ...
                    if (method_exists($this->loghistory_model, $phpfunc))
                    {
                        $data_arr = $this->loghistory_model->$phpfunc($data[$name], $mode, $id, $data, $parent_src, $this->term);
                    }
                }
                elseif (method_exists($this->general, $phpfunc))
                {
    ...
    public function getSelfSwitchTo()
    {
        ...
        $switchto_fields = $this->loghistory_model->switchto_fields;
        $extra_cond = $this->loghistory_model->extra_cond;
        ...
        $switch_arr = $this->loghistory_model->getSwitchTo($extra_cond);
    ...
    public function getListOptions()
    {
        ...
        $config_arr = $this->loghistory_model->getListConfiguration($alias_name);
