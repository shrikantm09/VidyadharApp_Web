...
class Group extends Cit_Controller
{
    ...
    public function index()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("group", $access_list, TRUE, TRUE);
            }
            if (!$list_access)
            {
    ...
    public function export()
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
        {
            $this->filter->checkAccessCapability("group_export");
        }
        else
        {
        ...
        $misc_info['heading'] = $this->lang->line('GROUP_GROUP');
        $misc_info['filename'] = "group_records_".count($db_recs);
        ...
        try
        {
            ...
            if ($mode == "Update")
            {
                ...
                else
                {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("group", $access_list, TRUE, TRUE);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("group", $access_list, TRUE, TRUE);
                }
                if (!$add_access)
                {
            ...
            if ($mode == 'Update')
            {
                ...
                if (isset($this->params_arr['switchIDs']) && $this->params_arr['switchIDs'] != "")
                {
                    ...
                    $switchCond = $this->db->protect($this->group_model->table_alias.".".$this->group_model->primary_key)." IN (".$switchEsc.")";
                    if ($extra_cond != "")
                    {
                ...
                foreach ($form_config as $key => $val)
                {
                    ...
                    if ($val['function'] != "")
                    {
                        ...
                        elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                        {
                            ...
                            {
                                $tmpdata = $this->group_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                        }
                    ...
                    if ($val['field_status'] != "")
                    {
                        ...
                        elseif ($status_type == "function")
                        {
                            ...
                            elseif (substr($fd_callback, 0, 7) == 'model::' && substr($fd_callback, 7) !== FALSE)
                            {
                                ...
                                if (method_exists($this->group_model, $fd_callback))
                                {
                                    $fd_status = $this->group_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
                            }
            ...
            if ($mode == "Update")
            {
                ...
                if (is_array($landing_page_arr) && count($landing_page_arr) > 0)
                {
                    ...
                    $landing_attr_selected = trim($landing_page_arr['phpFunc'] != '') ? $landing_page_arr['phpFunc'] : $landing_page_arr['menuItem'];
                }
                else
                {
            ...
            $this->dropdown->combo("array", "mgm_grouping_attr", $menu_items_opt, $landing_attr_selected);
            if ($this->config->item('ENABLE_ROLES_CAPABILITIES'))
            {
                $capability_masters = $group_capabilities = array();
                $capability_categories = $this->group_model->getCapabilityCategories();
                $list_masters_data = $this->group_model->getCapabilityMasters();
                foreach ($list_masters_data as $val)
                {
                    if ($val['eCapabilityType'] == "Module")
                    {
                        if (in_array($val['vCapabilityMode'], array("List", "View", "Add", "Update", "Delete", "Export", "Print")))
                        {
                            $capability_masters[$val['iCategoryId']]['module'][$val['vCapabilityMode']] = $val;
                        }
                        else
                        {
                            $capability_masters[$val['iCategoryId']]['custom']['others'][] = $val;
                        }
                    }
                    elseif ($val['vParentEntity'] == "settings")
                    {
                        if ($val['vCapabilityMode'] == "Update")
                        {
                            $capability_masters[$val['iCategoryId']]['custom']['settings']['update'][] = $val;
                        }
                        elseif ($val['vCapabilityMode'] == "View")
                        {
                            $capability_masters[$val['iCategoryId']]['custom']['settings']['view'][] = $val;
                        }
                        else
                        {
                            $capability_masters[$val['iCategoryId']]['custom']['others'][] = $val;
                ...
                if ($mode == "Update")
                {
                    $group_capabilities = $this->group_model->getGroupCapabilities($id);
                }
            }
            ...
            $render_arr = array(
                ...
                'capability_masters' => $capability_masters,
    ...
    public function addAction()
    {
        ...
        try
        {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                if ($mode == "Update")
                {
                    $add_edit_access = $this->filter->checkAccessCapability("group_update", TRUE);
                }
                else
                {
                    $add_edit_access = $this->filter->checkAccessCapability("group_add", TRUE);
                }
            }
            else
            {
            ...
            if ($this->config->item('ENABLE_ROLES_CAPABILITIES'))
            {
                $this->group_model->deleteGroupCapabilities($id);
                $capabilities_arr = $params_arr['capability'];
                $capability_json_arr = $params_arr['capability_json'];
                if (is_array($capabilities_arr) && count($capabilities_arr) > 0)
                {
                    foreach ($capabilities_arr as $key => $val)
                    {
                        $capability_json = "";
                        if (isset($capability_json_arr[$key]) && $capability_json_arr[$key] != "")
                        {
                            $capabilities = array(
                                "access_mode" => $capability_json_arr[$key],
                            );
                            $capability_json = json_encode($capabilities);
                        }
                        $insert_capability_arr = array(
                            "iGroupId" => $id,
                            "iCapabilityId" => $key,
                            "tCapabilities" => $capability_json,
                            "dtAddedDate" => date("Y-m-d H:i:s")
                        );
                        $this->group_model->insertGroupCapabilities($insert_capability_arr);
                    }
                }
            }
            else
        ...
        try
        {
            switch ($operartor)
            {
                case 'del':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $del_access = $this->filter->checkAccessCapability("group_delete", TRUE);
                    }
                    else
                    {
                ...
                case 'edit':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $edit_access = $this->filter->checkAccessCapability("group_update", TRUE);
                    }
                    else
                    {
