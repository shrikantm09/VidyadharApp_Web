...
/**
 * Description of Admin Notifications Controller
 *
 * @category admin
 *
 * @package user
 ...
 *
 * @class Admin_notifications.php
 *
 * @path application\admin\user\controllers\Admin_notifications.php
 *
 * @version 4.3
 *
 * @author CIT Dev Team
...
class Admin_notifications extends Cit_Controller
{
    /**
     * __construct method is used to set controller preferences while controller object initialization.
     * @created CIT Admin | 27.03.2019
     * @modified CIT Admin | 28.03.2019
     */
    public function __construct()
    {
        ...
        $this->module_name = "admin_notifications";
        ...
        $this->dropdown_arr = array(
            ...
            "man_is_read" => array(
                ...
                "values" => array(
                    array(
                        'id' => 'Yes',
                        'val' => $this->lang->line('ADMIN_NOTIFICATIONS_YES')
                    ),
                    array(
                        'id' => 'No',
                        'val' => $this->lang->line('ADMIN_NOTIFICATIONS_NO')
                    )
                )
            ),
            "man_status" => array(
                ...
                "values" => array(
                    array(
                        'id' => 'Active',
                        'val' => $this->lang->line('ADMIN_NOTIFICATIONS_ACTIVE')
                    ),
                    array(
                        'id' => 'Inactive',
                        'val' => $this->lang->line('ADMIN_NOTIFICATIONS_INACTIVE')
                    )
                )
            )
        );
        ...
        try
        {
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                $access_list = array(
                    ...
                    "admin_notifications_export",
                    "admin_notifications_print",
                );
            ...
            else
            {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("admin_notifications", $access_list, TRUE, TRUE);
            }
            if (!$list_access)
            {
                throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_VIEW_THIS_PAGE_C46_C46_C33'));
            }
            $enc_loc_module = $this->general->getMD5EncryptString("ListPrefer", "admin_notifications");
            ...
            $status_label = array(
                'js_lang_label.ADMIN_NOTIFICATIONS_YES',
                'js_lang_label.ADMIN_NOTIFICATIONS_NO',
            );

            $list_config = $this->admin_notifications_model->getListConfiguration();
            $this->processConfiguration($list_config, $add_access, $edit_access, TRUE);
            $this->general->trackModuleNavigation("Module", "List", "Viewed", $this->mod_enc_url["index"], "admin_notifications");
            ...
            $render_arr = array(
                ...
                'default_filters' => $this->admin_notifications_model->default_filters,
            );
            ...
            {
            $this->loadView("admin_notifications_index");
        }
        }
        catch(Exception $e)
        {
    ...
    public function listing()
    {
        ...
        $list_config = $this->admin_notifications_model->getListConfiguration();
        $form_config = $this->admin_notifications_model->getFormConfiguration();
        $extra_cond = $this->admin_notifications_model->extra_cond;
        $groupby_cond = $this->admin_notifications_model->groupby_cond;
        $having_cond = $this->admin_notifications_model->having_cond;
        $orderby_cond = $this->admin_notifications_model->orderby_cond;
        ...
        $data_recs = $this->admin_notifications_model->getListingData($data_config);
        $data_recs['no_records_msg'] = $this->general->processMessageLabel('ACTION_NO_ADMIN_NOTIFICATIONS_DATA_FOUND_C46_C46_C33');

        $data_recs = $this->admin_notifications_model->processCustomLinks($data_recs, $list_config);
    ...
    public function export()
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
        {
            $this->filter->checkAccessCapability("admin_notifications_export");
        }
        else
        {
            $this->filter->getModuleWiseAccess("admin_notifications", "Export", TRUE);
        }
        $params_arr = $this->params_arr;
        $page = $params_arr['page'];
        ...
        $list_config = $this->admin_notifications_model->getListConfiguration();
        $form_config = $this->admin_notifications_model->getFormConfiguration();
        $table_name = $this->admin_notifications_model->table_name;
        $table_alias = $this->admin_notifications_modeltable_alias;
        $primary_key = $this->admin_notifications_model->primary_key;
        $extra_cond = $this->admin_notifications_model->extra_cond;
        $groupby_cond = $this->admin_notifications_model->groupby_cond;
        $having_cond = $this->admin_notifications_model->having_cond;
        $orderby_cond = $this->admin_notifications_model->orderby_cond;
        ...
        $db_recs = $this->admin_notifications_model->getExportData($export_config);
        ...
        $misc_info['heading'] = $this->lang->line('ADMIN_NOTIFICATIONS_ADMIN_NOTIFICATIONS');
        $misc_info['filename'] = "admin_notifications_records_".count($db_recs);
        ...
        if ($export_type == 'pdf')
        {
            ...
            {
                $pdf->setModule("admin_notifications_model");
            }
        ...
        try
        {
            ...
            if ($mode == "Update")
            {
                ...
                else
                {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("admin_notifications", $access_list, TRUE, TRUE);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("admin_notifications", $access_list, TRUE, TRUE);
                }
                if (!$add_access)
                {
            ...
            if ($mode == 'Update')
            {
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowEdit", "admin_notifications"), $this->session->userdata('iAdminId'));
                $data_arr = $this->admin_notifications_model->getData(intval($id));
                ...
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "admin_notifications", $recName);
            }
            else
            {
                $recName = '';
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowAdd", "admin_notifications"), $this->session->userdata('iAdminId'));
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "admin_notifications");
            }

            $opt_arr = $img_html = $auto_arr = $config_arr = array();

            $form_config = $this->admin_notifications_model->getFormConfiguration($config_arr);
                ...
                foreach ($form_config as $key => $val)
                {
                    ...
                    if ($val['function'] != "")
                    {
                        ...
                        elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                        {
                            ...
                            if (method_exists($this->admin_notifications_model, $phpfunc))
                            {
                                $tmpdata = $this->admin_notifications_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                        }
                    ...
                    if ($val['field_status'] != "")
                    {
                        ...
                        elseif ($status_type == "function")
                        {
                            ...
                            elseif (substr($fd_callback, 0, 7) == 'model::' && substr($fd_callback, 7) !== FALSE)
                            {
                                ...
                                if (method_exists($this->admin_notifications_model, $fd_callback))
                                {
                                    $fd_status = $this->admin_notifications_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
                            }
            ...
            if ($mode == "Update")
            {
                ...
                {
                    $this->loadView("admin_notifications_add");
                }
                else
                {
                    $this->loadView("admin_notifications_add_view");
                }
            }
            else
            {
    ...
    public function addAction()
    {
        ...
        try
        {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                if ($mode == "Update")
                {
                    $add_edit_access = $this->filter->checkAccessCapability("admin_notifications_update", TRUE);
                }
                else
                {
                    $add_edit_access = $this->filter->checkAccessCapability("admin_notifications_add", TRUE);
                }
            }
            else
            {
                $add_edit_access = $this->filter->getModuleWiseAccess("admin_notifications", $mode, TRUE, TRUE);
            }
            if (!$add_edit_access)
            {
                if ($mode == "Update")
            ...
            $man_message = $params_arr["man_message"];
            $man_sent_by = $params_arr["man_sent_by"];
            $man_added_date = $params_arr["man_added_date"];
            $man_admin_id = $params_arr["man_admin_id"];
            $man_group_id = $params_arr["man_group_id"];
            $man_is_read = $params_arr["man_is_read"];
            $man_status = $params_arr["man_status"];

            $data = $save_data_arr = $file_data = array();
            $data["tMessage"] = $man_message;
            $data["iSentBy"] = $man_sent_by;
            $data["dAddedDate"] = $this->filter->formatActionData($man_added_date, $form_config["man_added_date"]);
            $data["iAdminId"] = $man_admin_id;
            $data["iGroupId"] = $man_group_id;
            $data["eIsRead"] = $man_is_read;
            $data["eStatus"] = $man_status;

            $save_data_arr["man_message"] = $data["tMessage"];
            $save_data_arr["man_sent_by"] = $data["iSentBy"];
            $save_data_arr["man_added_date"] = $data["dAddedDate"];
            $save_data_arr["man_admin_id"] = $data["iAdminId"];
            $save_data_arr["man_group_id"] = $data["iGroupId"];
            $save_data_arr["man_is_read"] = $data["eIsRead"];
            $save_data_arr["man_status"] = $data["eStatus"];
            if ($mode == 'Add')
            {
                $id = $this->admin_notifications_model->insert($data);
                if (intval($id) > 0)
                {
                    $save_data_arr["iNotificationId"] = $data["iNotificationId"] = $id;
                ...
                $track_cond = $this->db->protect("man.iNotificationId")." = ".$this->db->escape($id);
                $switch_combo = $this->admin_notifications_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Added", $this->mod_enc_url["add"], "admin_notifications", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            elseif ($mode == 'Update')
            {
                $res = $this->admin_notifications_model->update($data, intval($id));
                if (intval($res) > 0)
                {
                    $save_data_arr["iNotificationId"] = $data["iNotificationId"] = $id;
                ...
                $switch_combo = $this->admin_notifications_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Modified", $this->mod_enc_url["add"], "admin_notifications", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            $ret_arr['id'] = $id;
            $ret_arr['mode'] = $mode;
            $ret_arr['message'] = $msg;
    ...
    public function inlineEditAction()
    {
        ...
        if ($all_row_selected == "true" && in_array($operartor, array("del", "status")))
        {
            ...
            $config_arr['list_config'] = $this->admin_notifications_model->getListConfiguration();
            $config_arr['form_config'] = $this->admin_notifications_model->getFormConfiguration();
            $config_arr['table_name'] = $this->admin_notifications_model->table_name;
            $config_arr['table_alias'] = $this->admin_notifications_model->table_alias;
        ...
        if ($search_alias == "Yes")
        {
            $primary_field = $this->admin_notifications_model->table_alias.".".$this->admin_notifications_model->primary_key;
        }
        else
        {
            $primary_field = $this->admin_notifications_model->primary_key;
        }
        ...
        try
        {
            switch ($operartor)
            {
                case 'del':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $del_access = $this->filter->checkAccessCapability("admin_notifications_delete", TRUE);
                    }
                    else
                    {
                        $del_access = $this->filter->getModuleWiseAccess("admin_notifications", "Delete", TRUE, TRUE);
                    }
                    if (!$del_access)
                    {
                    ...
                    $success = $this->admin_notifications_model->delete($extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_FAILURE_IN_DELETION_THIS_RECORD_C46_C46_C33'));
                    }
                ...
                case 'edit':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $edit_access = $this->filter->checkAccessCapability("admin_notifications_update", TRUE);
                    }
                    else
                    {
                        $edit_access = $this->filter->getModuleWiseAccess("admin_notifications", "Update", TRUE, TRUE);
                    }
                    if (!$edit_access)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_MODIFY_THESE_DETAILS_C46_C46_C33'));
                    ...
                    $list_config = $this->admin_notifications_model->getListConfiguration($post_name);
                    $form_config = $this->admin_notifications_model->getFormConfiguration($list_config['source_field']);
                    if (!is_array($form_config) || count($form_config) == 0)
                    {
                    ...
                    $success = $this->admin_notifications_model->update($data_arr, intval($primary_ids));
                ...
                case 'status':
                    ...
                    {
                        $field_name = $this->admin_notifications_model->table_alias.".eIsRead";
                    }
                    else
                    {
                    ...
                    $success = $this->admin_notifications_model->update($data_arr, $extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
    ...
    public function getSourceOptions($name = '', $mode = 'Add', $id = '', $data = array(), $extra = '', $rtype = 'records')
    {
        ...
        switch ($type)
        {
            ...
            case 'table':
                ...
                {
                    $combo_config['main_table'] = $this->admin_notifications_model->table_name;
            ...
            case 'phpfn':
                ...
                elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                {
                    ...
                    if (method_exists($this->admin_notifications_model, $phpfunc))
                    {
                        $data_arr = $this->admin_notifications_model->$phpfunc($data[$name], $mode, $id, $data, $parent_src, $this->term);
                    }
                }
                elseif (method_exists($this->general, $phpfunc))
                {
    ...
    public function getSelfSwitchTo()
    {
        ...
        $switchto_fields = $this->admin_notifications_model->switchto_fields;
        $extra_cond = $this->admin_notifications_model->extra_cond;
        ...
        $switch_arr = $this->admin_notifications_model->getSwitchTo($extra_cond);
    ...
    public function getListOptions()
    {
        ...
        $config_arr = $this->admin_notifications_model->getListConfiguration($alias_name);
    ...
    public function notification_update()
    {
        try{
            ...
            $update_status = $this->admin_notifications_model->update($data, intval($id));
            if($update_status != true){
                throw new Exception('Notification Updation Failed');
            }
            $ret_arr['success'] = 1;
            $ret_arr['message'] = "Notification Updated Successfulley";
        } catch (Exception $e) {
            $message = $e;
            $ret_arr['success'] = 0;
            $ret_arr['message'] = $message;
