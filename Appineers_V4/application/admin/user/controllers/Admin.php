...
class Admin extends Cit_Controller
{
        ...
        $this->dropdown_arr = array(
            "ma_auth_type" => array(
                ...
                "values" => array(
                    array(
                        'id' => 'Google',
                        'val' => $this->lang->line('ADMIN_GOOGLE_AUTHENTICATOR')
                    ),
                    array(
                        'id' => 'Email',
                        'val' => $this->lang->line('ADMIN_EMAIL')
                    ),
                    array(
                        'id' => 'SMS',
                        'val' => $this->lang->line('ADMIN_SMS')
                    )
                )
            ),
            "ma_group_id" => array(
    ...
    public function index()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("admin", $access_list, TRUE, TRUE);
            }
            if (!$list_access)
            {
    ...
    public function listing()
    {
        ...
        if (method_exists($this->general, 'adminDataBeforeLoad'))
        {
            $data_recs = $this->general->adminDataBeforeLoad($data_recs, $this->admin_model->grid_fields, $list_config);
        }
    ...
    public function export()
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
        {
            $this->filter->checkAccessCapability("admin_export");
        }
        else
        {
        ...
        $misc_info['heading'] = $this->lang->line('ADMIN_ADMIN');
        $misc_info['filename'] = "admin_records_".count($db_recs);
        ...
        try
        {
            ...
            if ($mode == "Update")
            {
                ...
                else
                {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("admin", $access_list, TRUE, TRUE);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("admin", $access_list, TRUE, TRUE);
                }
                if (!$add_access)
                {
            ...
            if ($mode == 'Update')
            {
                ...
                if (isset($this->params_arr['switchIDs']) && $this->params_arr['switchIDs'] != "")
                {
                    ...
                    $switchCond = $this->db->protect($this->admin_model->table_alias.".".$this->admin_model->primary_key)." IN (".$switchEsc.")";
                    if ($extra_cond != "")
                    {
                ...
                foreach ($form_config as $key => $val)
                {
                    ...
                    if ($val['function'] != "")
                    {
                        ...
                        elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                        {
                            ...
                            {
                                $tmpdata = $this->admin_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                        }
                    ...
                    if ($val['field_status'] != "")
                    {
                        ...
                        elseif ($status_type == "function")
                        {
                            ...
                            elseif (substr($fd_callback, 0, 7) == 'model::' && substr($fd_callback, 7) !== FALSE)
                            {
                                ...
                                if (method_exists($this->admin_model, $fd_callback))
                                {
                                    $fd_status = $this->admin_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
                            }
    ...
    public function addAction()
    {
        ...
        try
        {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                if ($mode == "Update")
                {
                    $add_edit_access = $this->filter->checkAccessCapability("admin_update", TRUE);
                }
                else
                {
                    $add_edit_access = $this->filter->checkAccessCapability("admin_add", TRUE);
                }
            }
            else
            {
            ...
            $_pwd_encrypt = $this->general->encryptDataMethod($ma_password, "bcrypt");
            if ($_pwd_encrypt !== FALSE)
            {
                $data["vPassword"] = $_pwd_encrypt;
            }
            $data["eAuthType"] = $this->filter->formatActionData($ma_auth_type, $form_config["ma_auth_type"]);
            ...
            $save_data_arr["ma_auth_type"] = $data["eAuthType"];
            ...
            $save_data_arr["ma_auth_code"] = $data["vAuthCode"];
            ...
            $ret_arr['message'] = $msg;
            $ret_arr['success'] = 1;
            if (method_exists($this->general, 'sendAdminRegistrationEmail'))
            {
                $event_res = $this->general->sendAdminRegistrationEmail($mode, $id, $params_arr['parID'], $params_arr['parMod']);
        ...
        try
        {
            switch ($operartor)
            {
                case 'del':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $del_access = $this->filter->checkAccessCapability("admin_delete", TRUE);
                    }
                    else
                    {
                ...
                case 'edit':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $edit_access = $this->filter->checkAccessCapability("admin_update", TRUE);
                    }
                    else
                    {
            ...
            foreach ($search_arr as $key => $val)
            {
                ...
                else
                {
                    ...
                    if ($type == "single")
                    {
                        if (in_array($search_tbl_arr['type'], array("enum", "phpfn")))
                        {
                            $extra_cond = $display_query." IN ('".implode("','", $custom_arr)."')";
                        }
                        else
                        {
