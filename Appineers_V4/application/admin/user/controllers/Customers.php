...
/**
 * Description of Customers Controller
 *
 * @category admin
 *
 * @package user
 ...
 * @class Customers.php
 *
 * @path application\admin\user\controllers\Customers.php
 *
 * @version 4.4
 *
 * @author CIT Dev Team
...
class Customers extends Cit_Controller
{
    /**
        ...
        $this->dropdown_arr = array(
            "mc_status" => array(
                ...
                "values" => array(
                    array(
                        'id' => 'Active',
                        'val' => $this->lang->line('CUSTOMERS_ACTIVE')
                    ),
                    array(
                        'id' => 'Inactive',
                        'val' => $this->lang->line('CUSTOMERS_INACTIVE')
                    )
                )
            ),
            "mc_email_verified" => array(
                ...
                "values" => array(
                    array(
                        'id' => 'Yes',
                        'val' => $this->lang->line('CUSTOMERS_YES')
                    ),
                    array(
                        'id' => 'No',
                        'val' => $this->lang->line('CUSTOMERS_NO')
                    )
                )
            )
        );
    ...
    public function index()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("customers", $access_list, TRUE, TRUE);
            }
            if (!$list_access)
            {
                throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_VIEW_THIS_PAGE_C46_C46_C33'));
            }
            $enc_loc_module = $this->general->getMD5EncryptString("ListPrefer", "customers");
            ...
            $status_label = array(
                'js_lang_label.CUSTOMERS_ACTIVE',
                'js_lang_label.CUSTOMERS_INACTIVE',
            );

            $list_config = $this->customers_model->getListConfiguration();
            $this->processConfiguration($list_config, $add_access, $edit_access, TRUE);
            $this->general->trackModuleNavigation("Module", "List", "Viewed", $this->mod_enc_url["index"], "customers");
            ...
            $render_arr = array(
                ...
                'default_filters' => $this->customers_model->default_filters,
            );
    ...
    public function listing()
    {
        ...
        $list_config = $this->customers_model->getListConfiguration();
        $form_config = $this->customers_model->getFormConfiguration();
        $extra_cond = $this->customers_model->extra_cond;
        $groupby_cond = $this->customers_model->groupby_cond;
        $having_cond = $this->customers_model->having_cond;
        $orderby_cond = $this->customers_model->orderby_cond;
        ...
        $data_recs = $this->customers_model->getListingData($data_config);
        $data_recs['no_records_msg'] = $this->general->processMessageLabel('ACTION_NO__C35MODULE_NAME_C35_DATA_FOUND_C46_C46_C33');
    ...
    public function export()
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
        {
            $this->filter->checkAccessCapability("customers_export");
        }
        else
        {
            $this->filter->getModuleWiseAccess("customers", "Export", TRUE);
        }
        $params_arr = $this->params_arr;
        $page = $params_arr['page'];
        ...
        $list_config = $this->customers_model->getListConfiguration();
        $form_config = $this->customers_model->getFormConfiguration();
        $table_name = $this->customers_model->table_name;
        $table_alias = $this->customers_modeltable_alias;
        $primary_key = $this->customers_model->primary_key;
        $extra_cond = $this->customers_model->extra_cond;
        $groupby_cond = $this->customers_model->groupby_cond;
        $having_cond = $this->customers_model->having_cond;
        $orderby_cond = $this->customers_model->orderby_cond;
        ...
        $db_recs = $this->customers_model->getExportData($export_config);
        ...
        $misc_info['heading'] = $this->lang->line('CUSTOMERS_CUSTOMERS');
        $misc_info['filename'] = "customers_records_".count($db_recs);
        ...
        try
        {
            ...
            if ($mode == "Update")
            {
                ...
                else
                {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("customers", $access_list, TRUE, TRUE);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("customers", $access_list, TRUE, TRUE);
                }
                if (!$add_access)
                {
            ...
            if ($mode == 'Update')
            {
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowEdit", "customers"), $this->session->userdata('iAdminId'));
                $data_arr = $this->customers_model->getData(intval($id));
                ...
                if (isset($this->params_arr['switchIDs']) && $this->params_arr['switchIDs'] != "")
                {
                    ...
                    $switchCond = $this->db->protect($this->customers_model->table_alias.".".$this->customers_model->primary_key)." IN (".$switchEsc.")";
                    if ($extra_cond != "")
                    {
                ...
                $switch_arr = $this->customers_model->getSwitchTo($extra_cond, "records", $this->switchto_limit);
                $switch_combo = $this->filter->makeArrayDropDown($switch_arr);
                $switch_cit = array();
                $switch_tot = $this->customers_model->getSwitchTo($extra_cond, "count");
                if ($this->switchto_limit > 0 && $switch_tot > $this->switchto_limit)
                {
                    ...
                    {
                        $extra_cond = $this->db->protect($this->customers_model->table_alias.".".$this->customers_model->primary_key)." = ".$this->db->escape($id);
                        $switch_cur = $this->customers_model->getSwitchTo($extra_cond, "records", 1);
                ...
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "customers", $recName);
            }
            else
            {
                $recName = '';
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowAdd", "customers"), $this->session->userdata('iAdminId'));
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "customers");
            }

            $opt_arr = $img_html = $auto_arr = $config_arr = array();

            $form_config = $this->customers_model->getFormConfiguration($config_arr);
                ...
                foreach ($form_config as $key => $val)
                {
                    ...
                    if ($val['function'] != "")
                    {
                        ...
                        elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                        {
                            ...
                            if (method_exists($this->customers_model, $phpfunc))
                            {
                                $tmpdata = $this->customers_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                        }
                    ...
                    if ($val['field_status'] != "")
                    {
                        ...
                        elseif ($status_type == "function")
                        {
                            ...
                            elseif (substr($fd_callback, 0, 7) == 'model::' && substr($fd_callback, 7) !== FALSE)
                            {
                                ...
                                if (method_exists($this->customers_model, $fd_callback))
                                {
                                    $fd_status = $this->customers_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
                            }
            ...
            else
            {
                if ($mode == "Update")
                {
                    ...
                    {
                        $this->loadView("customers_add");
                    }
                    else
                    {
                        $this->loadView("customers_add_view");
                    }
                }
                else
                {
    ...
    public function addAction()
    {
        ...
        try
        {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                if ($mode == "Update")
                {
                    $add_edit_access = $this->filter->checkAccessCapability("customers_update", TRUE);
                }
                else
                {
                    $add_edit_access = $this->filter->checkAccessCapability("customers_add", TRUE);
                }
            }
            else
            {
                $add_edit_access = $this->filter->getModuleWiseAccess("customers", $mode, TRUE, TRUE);
            }
            if (!$add_edit_access)
            {
                if ($mode == "Update")
            ...
            $mc_first_name = $params_arr["mc_first_name"];
            $mc_last_name = $params_arr["mc_last_name"];
            $mc_email = $params_arr["mc_email"];
            $mc_user_name = $params_arr["mc_user_name"];
            $mc_password = $params_arr["mc_password"];
            $mc_profile_image = $params_arr["mc_profile_image"];
            $mc_status = $params_arr["mc_status"];
            $mc_email_verified = $params_arr["mc_email_verified"];
            $mc_registered_date = $params_arr["mc_registered_date"];

            $unique_arr = array();
            $unique_arr["vEmail"] = $mc_email;
            $unique_arr["vUserName"] = $mc_user_name;

            $unique_exists = $this->customers_model->checkRecordExists($this->customers_model->unique_fields, $unique_arr, $id, $mode, $this->customers_model->unique_type);
            if ($unique_exists)
            {
                $error_msg = $this->general->processMessageLabel('ACTION_RECORD_ALREADY_EXISTS_WITH_THESE_DETAILS_OF__C35FIELD_NAME_C35_C46_C46_C33');
                if ($error_msg == "")
                {
            ...
            $data["vFirstName"] = $mc_first_name;
            $data["vLastName"] = $mc_last_name;
            $data["vEmail"] = $mc_email;
            $data["vUserName"] = $mc_user_name;
            $_pwd_encrypt = $this->general->encryptDataMethod($mc_password, "bcrypt");
            if ($_pwd_encrypt !== FALSE)
            {
                $data["vPassword"] = $_pwd_encrypt;
            }
            $data["vProfileImage"] = $mc_profile_image;
            $data["eStatus"] = $mc_status;
            $data["eEmailVerified"] = $mc_email_verified;
            $data["dtRegisteredDate"] = $this->filter->formatActionData($mc_registered_date, $form_config["mc_registered_date"]);

            $save_data_arr["mc_first_name"] = $data["vFirstName"];
            $save_data_arr["mc_last_name"] = $data["vLastName"];
            $save_data_arr["mc_email"] = $data["vEmail"];
            $save_data_arr["mc_user_name"] = $data["vUserName"];
            $save_data_arr["mc_password"] = $data["vPassword"];
            $save_data_arr["mc_profile_image"] = $data["vProfileImage"];
            $save_data_arr["mc_status"] = $data["eStatus"];
            $save_data_arr["mc_email_verified"] = $data["eEmailVerified"];
            $save_data_arr["mc_registered_date"] = $data["dtRegisteredDate"];
            if ($mode == 'Add')
            {
                $id = $this->customers_model->insert($data);
                if (intval($id) > 0)
                {
                    $save_data_arr["iCustomerId"] = $data["iCustomerId"] = $id;
                ...
                $track_cond = $this->db->protect("mc.iCustomerId")." = ".$this->db->escape($id);
                $switch_combo = $this->customers_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Added", $this->mod_enc_url["add"], "customers", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            elseif ($mode == 'Update')
            {
                $res = $this->customers_model->update($data, intval($id));
                if (intval($res) > 0)
                {
                    $save_data_arr["iCustomerId"] = $data["iCustomerId"] = $id;
                ...
                $switch_combo = $this->customers_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Modified", $this->mod_enc_url["add"], "customers", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            $ret_arr['id'] = $id;
            $ret_arr['mode'] = $mode;
            $ret_arr['message'] = $msg;
            ...
            $file_data["mc_profile_image"]["file_name"] = $mc_profile_image;
            $file_data["mc_profile_image"]["old_file_name"] = $params_arr["old_mc_profile_image"];
            $file_data["mc_profile_image"]["unique_name"] = "mc_profile_image";
            $file_data["mc_profile_image"]["primary_key"] = "iCustomerId";
    ...
    public function inlineEditAction()
    {
        ...
        if ($all_row_selected == "true" && in_array($operartor, array("del", "status")))
        {
            ...
            $config_arr['list_config'] = $this->customers_model->getListConfiguration();
            $config_arr['form_config'] = $this->customers_model->getFormConfiguration();
            $config_arr['table_name'] = $this->customers_model->table_name;
            $config_arr['table_alias'] = $this->customers_model->table_alias;
        ...
        if ($search_alias == "Yes")
        {
            $primary_field = $this->customers_model->table_alias.".".$this->customers_model->primary_key;
        }
        else
        {
            $primary_field = $this->customers_model->primary_key;
        }
        ...
        try
        {
            switch ($operartor)
            {
                case 'del':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $del_access = $this->filter->checkAccessCapability("customers_delete", TRUE);
                    }
                    else
                    {
                        $del_access = $this->filter->getModuleWiseAccess("customers", "Delete", TRUE, TRUE);
                    }
                    if (!$del_access)
                    {
                    ...
                    if (is_array($primary_ids))
                    {
                        $main_cond = $this->db->protect("mc.iCustomerId")." IN ('".implode("','", $primary_ids)."')";
                    }
                    else
                    {
                        $main_cond = $this->db->protect("mc.iCustomerId")." = ".$this->db->escape($primary_ids);
                    }
                    $main_data = $this->customers_model->getData($main_cond, array('mc.vProfileImage'));
                    $success = $this->customers_model->delete($extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_FAILURE_IN_DELETION_THIS_RECORD_C46_C46_C33'));
                    }
                ...
                case 'edit':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $edit_access = $this->filter->checkAccessCapability("customers_update", TRUE);
                    }
                    else
                    {
                        $edit_access = $this->filter->getModuleWiseAccess("customers", "Update", TRUE, TRUE);
                    }
                    if (!$edit_access)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_MODIFY_THESE_DETAILS_C46_C46_C33'));
                    ...
                    $list_config = $this->customers_model->getListConfiguration($post_name);
                    $form_config = $this->customers_model->getFormConfiguration($list_config['source_field']);
                    if (!is_array($form_config) || count($form_config) == 0)
                    {
                    ...
                    $db_rec_arr = $this->customers_model->getData(intval($primary_ids));
                    $unique_arr = array();

                    $unique_arr[$field_name] = $post_val;
                    if (in_array($field_name, $this->customers_model->unique_fields))
                    {
                        $rec_arr = $this->customers_model->getData(intval($primary_ids), array('mc.vEmail', 'mc.vUserName'));
                        $unique_arr = is_array($rec_arr[0]) ? array_merge($rec_arr[0], $unique_arr) : $unique_arr;
                        $unique_exists = $this->customers_model->checkRecordExists($this->customers_model->unique_fields, $unique_arr, $primary_ids, "Update", $this->customers_model->unique_type);
                        if ($unique_exists)
                        {
                            $error_msg = $this->general->processMessageLabel('ACTION_RECORD_ALREADY_EXISTS_WITH_THESE_DETAILS_OF__C35FIELD_NAME_C35_C46_C46_C33');
                            if ($error_msg == "")
                    ...
                    $success = $this->customers_model->update($data_arr, intval($primary_ids));
                ...
                case 'status':
                    ...
                    {
                        $field_name = $this->customers_model->table_alias.".eStatus";
                    }
                    else
                    {
                    ...
                    $success = $this->customers_model->update($data_arr, $extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
            ...
            if ($mode == "Update")
            {
                ...
                if ($form_config["type"] == "file")
                {
                    ...
                    $file_data[$unique_name]["primary_key"] = "iCustomerId";
    ...
    public function getSourceOptions($name = '', $mode = 'Add', $id = '', $data = array(), $extra = '', $rtype = 'records')
    {
        ...
        switch ($type)
        {
            ...
            case 'table':
                ...
                {
                    $combo_config['main_table'] = $this->customers_model->table_name;
            ...
            case 'phpfn':
                ...
                elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                {
                    ...
                    if (method_exists($this->customers_model, $phpfunc))
                    {
                        $data_arr = $this->customers_model->$phpfunc($data[$name], $mode, $id, $data, $parent_src, $this->term);
                    }
                }
                elseif (method_exists($this->general, $phpfunc))
                {
    ...
    public function getSelfSwitchTo()
    {
        ...
        $switchto_fields = $this->customers_model->switchto_fields;
        $extra_cond = $this->customers_model->extra_cond;
        ...
        $switch_arr = $this->customers_model->getSwitchTo($extra_cond);
    ...
    public function getListOptions()
    {
        ...
        $config_arr = $this->customers_model->getListConfiguration($alias_name);
    ...
    public function downloadListFile()
    {
        ...
        if (is_array($config_arr) && count($config_arr) > 0)
        {
            ...
            $data_arr = $this->customers_model->getData(intval($id), $fields, '', '', '', 'Yes');
    ...
    public function uploadFormFile()
    {
        ...
        $config_arr = $this->customers_model->getFormConfiguration($unique_name);
    ...
    public function deleteFormFile()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                $res = $this->customers_model->update($update_arr, intval($id));
                if (!$res)
                {
