...
class Wsresponse
{
    ...
    public function sendWSResponse($arr = array(), $debug = array(), $res_format = 'json', $token = "")
    {
        ...
        if (is_array($exec_data) && count($exec_data) > 0) {
            ...
            if (isset($outputArr['output_response']['data'][0]['stripe_public_key'])) {
                $outputArr['output_response']['data'][0]["stripe_public_key"]= "******";
            }
    
            if (isset($outputArr['output_response']['data'][0]['google_places_key'])) {
                $outputArr['output_response']['data'][0]["google_places_key"]= "******";
            }
    
            if (isset($outputArr['output_response']['data'][0]['fcm_key'])) {
                $outputArr['output_response']['data'][0]["fcm_key"]= "******";
            }
            ...
            if ($exec_data['api_log_id'] > 0) {
                ...
                if ($this->CI->session->userdata('iUserId')) {
                    ...
                    //$updateArr['iPerformedBy'] = () ? $userid : NULL;
                    if (false == empty($userid)) {
                        $updateArr['iPerformedBy'] = $userid;
                    } else {
                        $this->CI->db->select('jInputParam');
                        $this->CI->db->where('iAccessLogId', $exec_data['api_log_id']);
                        $db_rec_obj = $this->CI->db->from('api_accesslogs')->get();
                        $result = is_object($db_rec_obj) ? $db_rec_obj->row_array() : array();
                        if (!empty($result)) {
                            $input_params = json_decode($result['jInputParam'], true);
                            $updateArr['iPerformedBy'] = $input_params['input_params']['post']['email'];
                        }
                    }
                }
                $updateArr['fExcutionTime'] = $this->CI->benchmark->elapsed_time('code_start', 'code_end');
                $updateArr['jOutputResponse'] = json_encode($outputArr);

                $updateArr['vStatusCode'] = http_response_code();
                if (http_response_code() == UNPROCESSABLE_ENTITY) {
                    $updateArr['vErrorType'] = "Unprocessable_Entity";
                } elseif (http_response_code() == UNAUTHORIZED) {
                    $updateArr['vErrorType'] = "Unauthorized";
                } elseif (http_response_code() == BAD_REQUEST) {
                    $updateArr['vErrorType'] = "Bad_request";
                } else {
                    $updateArr['vErrorType'] = "Executed";
                }
