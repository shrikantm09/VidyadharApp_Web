...
class User_sign_up_email extends Cit_Controller
{
    ...
    public function start_user_sign_up_email($request_arr = array(), $inner_api = false)
    {
        try {
            ...
            if ($condition_res["success"]) {
                ...
                $condition_res_1 = $this->check_device_token_status($input_params);
                if ($condition_res_1["success"]) {
                    $input_params = $this->remove_device_token($input_params);
                }
                $input_params = $this->create_user($input_params);
                if ($input_params["create_user"]["success"]==1){
     ...
     * @return array $block_result returns result of condition block as array.
     */
    public function check_device_token_status($input_params = array())
    {
        $this->block_result = array();
        try {
            $cc_lo_0 = $input_params["status"];
    ...
    /**
     * remove other device token method is used to process query block.
     * @param array $input_params input_params array to process loop flow.
     * 
     * @return array $input_params returns modfied input_params array.
     */
