...
class Edit_profile extends Cit_Controller
{
   ...
   public $block_result;
            ...
            if ($condition_res["success"])
            {
                       ...
                       if ($condition_res["success"]) {
                           ...
                           if ($condition_res["success"]) {
                            ...
                            if (!empty($input_params['delete_image_ids'])) {
                                $input_params = $this->get_user_media_image($input_params);
                                if (!empty($input_params['image_names']['image_id'])) {
    ...
    /**
     * This method is used to get user media images.
     * 
     * @param array $input_params input_params array to process condition flow.
     * 
     * @return array $block_result returns result of condition block as array.
    ...
    public function get_user_media_image($input_params = array())
    {
        ...
        try
        {
            ...
            if (!empty($input_params["delete_image_ids"])) {
                ...
                if (isset($input_params["delete_image_ids"])) {
                    $where_arr['image_id']  = explode(",", $input_params["delete_image_ids"]);
                
                    $this->block_result = $this->users_model->get_user_personal_images($where_arr);

                
                    if (!$this->block_result["success"]) {
                        throw new Exception("media images not found.");
                    }

                    $result_arr = $this->block_result["data"];

                  
                    if (is_array($result_arr["image_id"]) && count($result_arr["image_id"]) > 0) {

                        $result = array_diff($where_arr['image_id'], $result_arr["image_id"]);

                     if(count($result)>0)
                     {
                        $result1=implode(",",$result);
                        throw new Exception("invalid image id $result1");
        ...
        $input_params["image_names"] = $this->block_result["data"];
    ...
    /**
     * This method is used to delete user images.
     * 
     * @param array $input_params input_params array to process loop flow.
     * 
     * @return array $input_params returns modfied input_params array.
      ...
      $this->block_result = array();
        try
        {
            ...
            if (!empty($input_params["delete_image_ids"])) {
                ...
                $this->block_result = $this->users_model->delete_user_media_image($where_arr);
            
                if (!$this->block_result["success"]) {
                    throw new Exception("failed to delete media images.");
                }

                if (count($input_params["image_names"]["image_url"]) > 0) {
                    $delete_images = $input_params["image_names"]["image_url"];

                    foreach ($delete_images as $image_name) {
                        $data11 = $this->general->deleteAWSFileData($folder_name = $aws_folder_name . "/user_profile/" . $input_params['user_id']);
                    }
                }
            }
     ...
     * @return array $responce_arr returns responce array of api.
     */
    public function users_finish_success_5($input_params)
    {
     ...
     * @return array $responce_arr returns responce array of api.
     */
    public function users_finish_success_6($input_params)
    {

        $setting_fields = array(
            "success" => FAILED_CODE,
            "message" => "User image deletion fail",
        );
        $output_fields = array();
