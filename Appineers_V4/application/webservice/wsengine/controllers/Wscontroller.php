       ...
       $api_allowed_without_access_token = array('social_login','user_login_phone','user_login_email','send_verification_link','check_unique_user','social_sign_up','user_sign_up_phone','user_sign_up_email','user_email_confirmation','static_pages','send_sms','reset_password_confirmation','reset_password_phone','get_template_message','forgot_password_phone','reset_password','forgot_password','states_list','get_config_paramaters','category_list','category_with_sub');
        ...
        if ($appFailled == 'No' and !in_array($func_name, $api_allowed_without_access_token)) {
            ...
            if ($req_token == '') {
                ...
                $this->wsresponse->setResponseStatus(BAD_REQUEST);
            ...
            if ($req_token) {
                $access = $req_token;
                $this->db->select('iUserId,eStatus,eDeviceType,vDeviceModel,vDeviceOS,vTermsConditionsVersion,vPrivacyPolicyVersion,vAppVersion');
                $this->db->from('users');
                $this->db->where('vAccessToken', $access);
                $result = $this->db->get()->result_array();
                $userid = $result[0]['iUserId'];
                $status = $result[0]['eStatus'];
                $DeviceType = $result[0]['eDeviceType'];
                $DeviceModel = $result[0]['vDeviceModel'];
                $DeviceOS = $result[0]['vDeviceOS'];
                $TermsConditionsVersion = $result[0]['vTermsConditionsVersion'];
                $PrivacyPolicyVersion = $result[0]['vPrivacyPolicyVersion'];
                $AppVersion = $result[0]['vAppVersion'];
            }

            if (!empty($userid) && $status =='Active') {
                $request_arr['user_id']=$userid;
                $request_arr['DeviceType']=$DeviceType;
                $request_arr['DeviceModel']=$DeviceModel;
                $request_arr['DeviceOS']=$DeviceOS;
                $request_arr['TermsConditionsVersion']=$TermsConditionsVersion;
                $request_arr['PrivacyPolicyVersion']=$PrivacyPolicyVersion;
                $request_arr['AppVersion']=$AppVersion;
            } elseif (!empty($userid) && $status =='Inactive') {
                $appFailled = "Yes";
                $code = "0";
                ...
                $appFailled = "Yes";
                $code = "0";
                $msg = "The user does not have a valid access token.";
                $this->wsresponse->setResponseStatus(SESSION_EXPIRE);
            }
            ...
            if ($req_token) {
                $access = $req_token;
                $this->db->select('iUserId,eDeviceType,vDeviceModel,vDeviceOS,vTermsConditionsVersion,vPrivacyPolicyVersion,vAppVersion');
                $this->db->from('users');
