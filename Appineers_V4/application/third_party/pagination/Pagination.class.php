...
class Pagination
{
    ...
    public function getCanonicalUrl()
    {
        ...
        if (empty($target)) {
            $target = 'http://' . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'];
        }
        $page = (int) $this->_variables['current'];
        if ($page !== 1) {
            return $target . $this->getPageParam();
        }
        return $target;
    }
    ...
    public function getPageUrl($page = false)
    {
        ...
        return $target . $this->getPageParam($page);
    }
