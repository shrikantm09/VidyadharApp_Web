...
class oauth_client_class
{
                    ...
                    '{CLIENT_ID}', UrlEncode($this->client_id), str_replace(
                        '{API_KEY}', UrlEncode($this->api_key), str_replace(
                            '{SCOPE}', UrlEncode($this->scope), str_replace(
                                '{REALM}', UrlEncode($this->realm), $url))))));
        return(true);
    }
    ...
    Function SendAPIRequest($url, $method, $parameters, $oauth, $options, &$response)
    {
        ...
        if (strlen($authorization) === 0 && !strcasecmp($this->access_token_type, 'Bearer') && intval($this->oauth_version) == 1)
            $authorization = 'Bearer ' . $this->access_token;
        elseif (intval($this->oauth_version) == 2)
        ...
        switch ($authentication = (IsSet($options['AccessTokenAuthentication']) ? strtolower($options['AccessTokenAuthentication']) : '')) {
            ...
            case 'bearer':
                if (strlen($authorization)) {
                    $arguments['Headers']['Authorization'] = 'Bearer ' . $authorization;
                }
                break;
            //HB Customization >> End
            case '':
    ...
    Function ProcessToken2($code, $refresh)
    {
        ...
        //HB Customization << Start
        $this->instance_url = str_replace(array('https://', 'http://'), '', (strlen($response[$this->instance_url_parameter]) ? $response[$this->instance_url_parameter] : $response['instance_url']));
        //HB Customization >> End
        $access_token = array(
        ...
        //HB Customization << Start
        if ($response[$this->instance_url_parameter] != '') {
            $access_token[$this->instance_url_parameter] = $this->instance_url;
        }
        //HB Customization >> End
    ...
    //HB Customization << Start
    Function CallAPICustom($url, $method, $parameters, $options, $headers, &$response)
    {
        if (!IsSet($options['Resource']))
            $options['Resource'] = 'API call';
        if (!IsSet($options['ConvertObjects']))
        ...
        return($this->SendAPIRequestBypass($url, $method, $parameters, $oauth, $options, $headers, $response));
    }

    Function SendAPIRequestBypass($url = '', $method, $parameters, $oauth, $options, $headers, &$response)
    {
        $this->response_status = 0;
        $http = new http_class;
        $http->debug = ($this->debug && $this->debug_http);
        ...
        if ($method == 'JSONPOST') {
            $method = 'POST';
            $type = $options['RequestContentType'] = 'application/json';
        }
        if (1 || IsSet($oauth)) {
            $files = (IsSet($options['Files']) ? $options['Files'] : array());
            if (count($files)) {
            ...
            $key = $this->Encode($this->client_secret) . '&' . $this->Encode($this->access_token_secret);
            $post_values = $parameters;
            if (1 || $this->authorization_header) {
                $authorization = 'OAuth';
                $first = true;
                foreach ($values as $parameter => $value) {
                    $authorization .= ($first ? ' ' : ',') . $parameter . '="' . $this->Encode($value) . '"';
        ...
        if (!empty($headers)) {
            foreach ($headers as $key => $val) {
                $arguments['Headers'][$key] = urldecode(trim($val));
            }
        }
        if (strlen($error = $http->SendRequest($arguments)) || strlen($error = $http->ReadReplyHeaders($headers))) {
            $http->Close();
    ...
    Function Initialize()
    {
        if (strlen($this->server) === 0)
            return true;
        $this->oauth_version = $this->dialog_url = $this->reauthenticate_dialog_url = $this->pin_dialog_url = $this->access_token_url = $this->request_token_url = $this->append_state_to_redirect_uri = '';
    ...
    //HB Customization << Start
    Function getFullToken($sess_code, $method, $parameters, $options, &$response, $service_url = '')
    {
        if (!IsSet($options['Resource']))
            $options['Resource'] = 'API call';
        if (!IsSet($options['ConvertObjects']))
        ...
        switch (intval($this->oauth_version)) {
            ...
            case 2:
                ...
                $oauth = null;
                if (strcasecmp($this->access_token_type, 'Bearer')) {
                    $_SESSION[$sess_code][(strlen($this->access_token_parameter) ? $this->access_token_parameter : 'access_token')] = UrlEncode($this->access_token);
                    if ($this->instance_url_parameter != '') {
                        $_SESSION[$sess_code][$this->instance_url_parameter] = urldecode($this->instance_url);
                    }
        ...
        return($this->SendAPIRequestCustom($url, $method, $parameters, $oauth, $options, $response, $sess_code, $service_url));
    }

    Function SendAPIRequestCustom($url = '', $method, $parameters, $oauth, $options, &$response, $sess_code, $service_url = '')
    {
        $this->response_status = 0;
        $http = new http_class;
        $http->debug = ($this->debug && $this->debug_http);
        ...
        if (IsSet($oauth)) {
            ...
            if ($this->authorization_header) {
                ...
                if ($method === 'GET' || (IsSet($options['PostValuesInURI']) && $options['PostValuesInURI'])) {
                    ...
                    $temp_url = '';
                    foreach ($values as $parameter => $value) {
                        $temp_url .= ($first ? '?' : '&') . $parameter . '=' . $this->Encode($value);
                        $first = false;
                    }
                    $post_values = $values;
                } else {
        ...
        switch (IsSet($options['AccessTokenAuthentication']) ? strtolower($options['AccessTokenAuthentication']) : '') {
            case 'basic':
                $arguments['Headers']['Authorization'] = 'Basic ' . base64_encode($this->client_id . ':' . ($this->get_token_with_api_key ? $this->api_key : $this->client_secret));
                $_SESSION[$sess_code]['access_token'] = 'Basic ' . base64_encode($this->client_id . ':' . ($this->get_token_with_api_key ? $this->api_key : $this->client_secret));
                return "token_authenticated";
                break;
            case 'bearer':
                if (strlen($authorization)) {
                    $arguments['Headers']['Authorization'] = $authorization;
                    $_SESSION[$sess_code]['access_token'] = $authorization;
                    $_SESSION[$sess_code][(strlen($this->instance_url_parameter) ? $this->instance_url_parameter : 'instance_url')] = urldecode($this->instance_url);
                    if ($method === 'GET') {
                        $_SESSION[$sess_code]['url_patch'] = $temp_url;
                    } else {
                        $_SESSION[$sess_code]['url_patch'] = $post_values;
                    }

                    return "token_authenticated";
                } else {
                    $_SESSION[$sess_code]['access_token'] = $this->access_token;
                    $_SESSION[$sess_code]['instance_url'] = $this->instance_url;
                    $_SESSION[$sess_code]['url_patch'] = $post_values;
                    return "token_authenticated";
                }
                break;
            case '':
                if (strlen($authorization)) {
                    //$arguments['Headers']['Authorization'] = $authorization;
                    $_SESSION[$sess_code]['access_token'] = $this->access_token;
                    $_SESSION[$sess_code]['access_token_secret'] = $this->access_token_secret;
                    if ($method === 'GET') {
                        $_SESSION[$sess_code]['url_patch'] = $temp_url;
                    } else {
                        $_SESSION[$sess_code]['url_patch'] = $post_values;
